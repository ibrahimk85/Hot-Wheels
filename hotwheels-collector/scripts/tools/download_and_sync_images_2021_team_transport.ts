/**
 * Script to download image assets for the 2021 Hot Wheels Team Transport set and
 * associate them with Model and Variant records in your database.
 *
 * This script performs two jobs:
 *   1. Fetches the 2021 Team Transport page from the Hot Wheels Fandom wiki and
 ...
 *      `public/images/hotwheels/2021/team-transport/` folder.
 ...
 *     the 2021 Team Transport variants using the import script.
 ...
 *
 * Usage:
 *   npx ts-node scripts/tools/download_and_sync_images_2021_team_transport.ts
 */
...
const TEAM_TRANSPORT_URL = 'https://hotwheels.fandom.com/wiki/2021_Car_Culture:_Team_Transport';
...
async function main() {
  ...
  for (let tableIdx = 0; tableIdx < tables.length; tableIdx++) {
    ...
    // Special handling for "Supreme Exclusive", "Walmart Legends Tour Exclusive", or "Fast & Furious"
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      // Transport row pattern: First cell is Toy# (short alphanumeric), second is Series# (digit or #digit or N/A)
      ...
      if (firstCell.length >= 3 && firstCell.length <= 8 && /^[A-Z0-9]+$/i.test(firstCell) && !firstCell.includes(' ') &&
        ...
        // Car row pattern: First cell is Casting Name
        if (cells.length > 0) {
          const cell = $(cells[0]);
          const link = cell.find('a').first();
          if (link.length > 0) {
    ...
    for (const [setKey, setRows] of groupedBySet.entries()) {
      ...
      for (let carIdx = 0; carIdx < carRows.length; carIdx++) {
        ...
        if (!carRow.photoLooseUrl) {
          console.warn(`  Car row ${carIdx + 1}/${carRows.length} (${carRow.castingName}) has no Photo Loose URL`);
          continue;
        }
