...
const WIKI_URL = 'https://hotwheels.fandom.com/wiki/Pixar_Series_(2020)';
const COLLECTION_NAME = 'Hot Wheels Silver Series';
const SERIES_NAME = 'Pixar (2020)';
const BASE_SUBDIR = 'silver-series/pixar';
...
// Pixar: Col# | Casting | Color | Theme | Wheel Type | Toy# | Notes | Photo Loose | Photo Carded
function parseRow($: cheerio.CheerioAPI, cells: cheerio.Cheerio<any>) {
  const isPixar = cells.length >= 9;
  const colNumber = cells.length > 0 ? $(cells[0]).text().trim() : '';
  const toyNumber = sanitize(isPixar && cells.length > 5 ? $(cells[5]).text().trim() : (cells.length > 1 ? $(cells[1]).text().trim() : ''));
  const castIdx = isPixar ? 1 : 2;
  ...
  const color = cells.length > 2 ? $(cells[2]).text().trim() : '';
  return { toyNumber, colNumber, castingName, color };
}

async function main() {
  console.log('=== Pixar (2020) IMAGE SCRIPT ===');
  ...
  let dl = 0, assoc = 0;
  const getImageIndices = (n: number) => n >= 10 ? { loose: 8, carded: 9 } : { loose: 7, carded: 8 };
