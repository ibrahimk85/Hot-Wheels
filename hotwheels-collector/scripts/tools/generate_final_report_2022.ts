/**
 * Generate final report for 2022 scraping
 */
...
async function main() {
  ...
  const year2022 = await prisma.year.findFirst({ where: { year: 2022 } });
  if (!year2022) {
    console.log('⚠️  Year 2022 not found. Please run the scraping script first.');
    await prisma.$disconnect();
    return;
  }

  const mainlineCollection = await prisma.collection.findFirst({
    where: { name: 'Mainline', yearId: year2022.id },
    include: {
      _count: {
        select: {
          models: true,
  ...
  if (!mainlineCollection) {
    console.log('⚠️  Mainline collection not found for 2022.');
    await prisma.$disconnect();
    return;
  }
  ...
  const report = `# 2022 Hot Wheels Mainline Scraping Final Raporu
...
`;
  ...
  const reportPath = path.join(process.cwd(), 'scripts', 'tools', 'SCRAPE_2022_REPORT.md');
