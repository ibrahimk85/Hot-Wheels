/**
 * Download Fast & Furious Premium wiki images for 2026 (403-safe fetch + CDN binary download).
 * Skips Tokyo Drift Bundle table (same as import).
 *
 *   npx ts-node scripts/tools/download_and_sync_images_2026_fast_furious_premium.ts
 */
...
async function main() {
  ...
  const yearTables = allTables.filter((_, table) => {
    ...
    if (/tokyo drift bundle/i.test(headingText)) return false;
    return true;
  });
