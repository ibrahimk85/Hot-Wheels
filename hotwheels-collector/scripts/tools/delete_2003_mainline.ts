...
async function delete2003Mainline() {
  console.log('=== Delete 2003 Mainline Data & Images ===\n');
  ...
  console.log(`Models under 2003 Mainline: ${modelIds.length}`);
  ...
  console.log(`Remaining variants for 2003 Mainline: ${remainingVariants}`);
  ...
  if (remainingVariants === 0 && remainingImages === 0 && !folderExistsAfter) {
    console.log('\n✅ 2003 Mainline data and images removed.');
  } else {
    console.log('\n⚠ Some 2003 Mainline data or images may still remain. Review logs.');
  }
}

(async () => {
  try {
    await delete2003Mainline();
  } catch (err) {
    console.error('❌ Error while deleting 2003 Mainline:', err);
    process.exitCode = 1;
  } finally {
