/**
 * Script to download image assets for the 2015 Hot Wheels Fast & Furious Series.
 * 
 * This script:
 *   1. Fetches the Fast & Furious Series page from the Hot Wheels Fandom wiki
 *   2. Extracts Photo - Card and Photo - Car image URLs for 2015 year table
 *   3. Downloads images to public/images/hotwheels/2015/fast-and-furious/{castingSlug}/
 *   4. Associates images with Variant records
 * 
 * Fast & Furious Series-specific:
 * - Photo - Card (Carded) → Main image (variant.imageId)
 * - Photo - Car (Loose) → Second image (variant.images[])
 * - File names: {toyNumber}_carded.jpg and {toyNumber}_loose.jpg
 * - Variant matching: Year + Series # + Casting Name + Color + SubSeries
 * 
 * 2015 Table Structure (9 columns):
 ...
 * 
 * How to use:
 *   npx ts-node scripts/tools/download_and_sync_images_2015_fast_and_furious.ts
 */
...
async function main() {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    // Process Photo - Card (Carded) (main image) - Column 8
    ...
    // Process Photo - Car (Loose) (second image) - Column 7
