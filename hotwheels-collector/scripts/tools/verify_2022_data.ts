/**
 * Verification script to check 2022 data
 */
...
async function main() {
  console.log('🔍 Verifying 2022 Mainline Data...\n');

  // Check Year
  const year2022 = await prisma.year.findFirst({ where: { year: 2022 } });
  console.log(`Year 2022: ${year2022 ? '✅ Exists' : '❌ Not found'}`);

  if (!year2022) {
    console.log('\n⚠️  Year 2022 not found. The scraping script may not have run yet.');
  ...
  if (!mainlineCollection) {
    console.log('\n⚠️  Mainline collection not found for 2022.');
    await prisma.$disconnect();
    return;
  }
