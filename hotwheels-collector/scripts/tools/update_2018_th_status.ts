/**
 * Script to update 2018 Treasure Hunt and Super Treasure Hunt status
 * by scraping the Hot Wheels Fandom wiki page and matching entries
 * with database variants.
 *
 * This script:
 * 1. Fetches the List of 2018 Hot Wheels page from hotwheels.fandom.com
 ...
 *
 * Usage:
 *   npx ts-node scripts/tools/update_2018_th_status.ts
 */
...
async function updateVariants() {
  console.log('=== 2018 Treasure Hunt Status Update ===\n');

  const variantCount = await prisma.variant.count({
    where: { year: 2018 },
  });
  console.log(`Found ${variantCount} variants in database for year 2018\n`);

  if (variantCount === 0) {
    console.warn('⚠ No 2018 variants found in database. Please import 2018 mainline data first.');
    return;
  }
