/**
 * Download images for 2022 Disney Classics (Entertainment) - 8-col
 * npx ts-node scripts/tools/download_and_sync_images_2022_silver_series_disney_classics.ts
 */
...
const WIKI_URL = 'https://hotwheels.fandom.com/wiki/Tokyo_2020_Olympics_Series_(2021)';
const COLLECTION_NAME = 'Hot Wheels Silver Series';
const SERIES_NAME = 'Tokyo 2020 Olympics (2021)';
const BASE_SUBDIR = 'silver-series/tokyo-olympics';
...
// Tokyo Olympics: Series # | Casting | Toy # | Color | Sport | ... | Photo | Photo Card
function parseRow($: cheerio.CheerioAPI, cells: cheerio.Cheerio<any>) {
  const isTokyo = cells.length >= 12;
  const colNumber = cells.length > 0 ? $(cells[0]).text().trim() : '';
  const toyNumber = sanitize(isTokyo && cells.length > 2 ? $(cells[2]).text().trim() : (cells.length > 1 ? $(cells[1]).text().trim() : ''));
  const castIdx = isTokyo ? 1 : 2;
  const link = $(cells[castIdx]).find('a').first();
  const castingName = link.length > 0 ? link.text().trim() : cells.length > castIdx ? $(cells[castIdx]).text().trim() : '';
  const color = cells.length > 3 ? $(cells[3]).text().trim() : '';
  return { toyNumber, colNumber, castingName, color };
}

async function main() {
  console.log('=== Tokyo 2020 Olympics (2021) IMAGE SCRIPT ===');
  ...
  let dl = 0, assoc = 0;
  const getImageIndices = (n: number) => n >= 12 ? { loose: 11, carded: 12 } : (n >= 10 ? { loose: 8, carded: 9 } : { loose: 7, carded: 8 });
