...
async function delete2005Mainline() {
  console.log('=== Delete 2005 Mainline Data & Images ===\n');
  ...
  console.log(`Models under 2005 Mainline: ${modelIds.length}`);
  ...
  console.log(`Remaining variants for 2005 Mainline: ${remainingVariants}`);
  ...
  if (remainingVariants === 0 && remainingImages === 0 && !folderExistsAfter) {
    console.log('\n✅ 2005 Mainline data and images removed.');
  } else {
    console.log('\n⚠ Some 2005 Mainline data or images may still remain. Review logs.');
  }
}

(async () => {
  try {
    await delete2005Mainline();
  } catch (err) {
    console.error('❌ Error while deleting 2005 Mainline:', err);
    process.exitCode = 1;
  } finally {
