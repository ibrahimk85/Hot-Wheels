/**
 * Script to analyze Neon Speeders 2024 wiki page structure
 */
...
async function main() {
  ...
  console.log(`Analyzing 2024 section in ${wikiUrl}\n`);

  const html = await fetchWithRetry(wikiUrl);
  const $ = cheerio.load(html);

  // Find heading "2024"
  const heading2024 = $('h2, h3, h4').filter((_, el) => {
    const text = $(el).text().trim();
    return text.includes('2024') && !text.includes('2023');
  }).first();

  console.log(`Found heading for 2024: "${heading2024.text().trim()}"`);
  console.log(`Heading level: ${heading2024.prop('tagName')}\n`);

  // Find all tables after 2024 heading until next year heading
  const nextYearHeading = heading2024.nextAll('h2, h3, h4').filter((_, el) => {
    const text = $(el).text().trim();
    return /^\d{4}/.test(text) && !text.includes('2024');
  }).first();

  let tablesToCheck;
  if (nextYearHeading.length > 0) {
    tablesToCheck = heading2024.nextUntil(nextYearHeading).filter('table.wikitable');
  } else {
    tablesToCheck = heading2024.nextAll('table.wikitable');
  }

  console.log(`Found ${tablesToCheck.length} table(s) for 2024\n`);
