...
const WIKI_URL = 'https://hotwheels.fandom.com/wiki/Batman_Series_(2019)';
const COLLECTION_NAME = 'Hot Wheels Silver Series';
const SERIES_NAME = 'Batman (2019)';
const BASE_SUBDIR = 'silver-series/batman-2019';
...
async function main() {
  console.log('=== Batman (2019) IMAGE SCRIPT ===');
  ...
  let dl = 0, assoc = 0;
  // Batman 7-col: Col# | Toy# | Casting | Body Color | Wheel Type | Notes | Photo Loose | Photo Carded (8 cells)
const getImageIndices = (n: number) => n >= 12 ? { loose: 11, carded: 12 } : (n >= 10 ? { loose: 8, carded: 9 } : (n === 8 ? { loose: 6, carded: 7 } : { loose: 7, carded: 8 }));
