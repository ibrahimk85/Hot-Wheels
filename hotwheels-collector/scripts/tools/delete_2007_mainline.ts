...
async function delete2007Mainline() {
  console.log('=== Delete 2007 Mainline Data & Images ===\n');
  ...
  console.log(`Models under 2007 Mainline: ${modelIds.length}`);
  ...
  console.log(`Remaining variants for 2007 Mainline: ${remainingVariants}`);
  ...
  if (remainingVariants === 0 && remainingImages === 0 && !folderExistsAfter) {
    console.log('\n✅ 2007 Mainline data and images removed.');
  } else {
    console.log('\n⚠ Some 2007 Mainline data or images may still remain. Review logs.');
  }
}

(async () => {
  try {
    await delete2007Mainline();
  } catch (err) {
    console.error('❌ Error while deleting 2007 Mainline:', err);
    process.exitCode = 1;
  } finally {
