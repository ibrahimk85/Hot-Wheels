/**
 * Fix Boulevard duplicate models for 2012
 * 
 * Strategy:
...
async function main() {
  ...
  if (duplicates.length > 0) {
    ...
    for (const [castingName, models] of duplicates) {
      // Valid Mix names for 2012
      const validMixNames = ['Ahead of Its Time', 'Big Hits', 'Concept Cars', 'Legends', 'Show Rods', 'Underdogs'];
      
      // Sort: first by valid Mix name, then by hasSubSeries, then by hasImages
      const sorted = models.sort((a, b) => {
        const aIsValidMix = a.subSeries?.name && validMixNames.includes(a.subSeries.name) ? 1 : 0;
        const bIsValidMix = b.subSeries?.name && validMixNames.includes(b.subSeries.name) ? 1 : 0;
        if (aIsValidMix !== bIsValidMix) {
          return bIsValidMix - aIsValidMix; // Valid Mix first
        }
