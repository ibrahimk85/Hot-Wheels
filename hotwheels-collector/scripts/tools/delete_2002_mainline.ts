...
async function delete2002Mainline() {
  console.log('=== Delete 2002 Mainline Data & Images ===\n');
  ...
  console.log(`Models under 2002 Mainline: ${modelIds.length}`);
  ...
  console.log(`Remaining variants for 2002 Mainline: ${remainingVariants}`);
  ...
  if (remainingVariants === 0 && remainingImages === 0 && !folderExistsAfter) {
    console.log('\n✅ 2002 Mainline data and images removed.');
  } else {
    console.log('\n⚠ Some 2002 Mainline data or images may still remain. Review logs.');
  }
}

(async () => {
  try {
    await delete2002Mainline();
  } catch (err) {
    console.error('❌ Error while deleting 2002 Mainline:', err);
    process.exitCode = 1;
  } finally {
