/**
 * Download 2000 Mainline images from Fandom and attach to Variant rows by Toy#.
 *
 * Skips Walmart Redline Exclusives and Goodyear Tire Exclusives tables if present (same rules as import).
 *
 *   npx ts-node scripts/tools/download_and_sync_images_2000_mainline.ts
 *
 * Wiki HTML fallback: set FANDOM_WIKI_HTML_PATH to a saved List_of_2000 page (same as import).
 */
...
const MAINLINE_URL = 'https://hotwheels.fandom.com/wiki/List_of_2000_Hot_Wheels';
const TARGET_YEAR = 2000;

function findPrevTableHeading($table: cheerio.Cheerio<any>, $: cheerio.CheerioAPI): string | null {
  let el = $table.prev();
  for (let d = 0; d < 5 && el.length; d++) {
    const tag = el[0]?.tagName?.toLowerCase();
    if (tag === 'h2' || tag === 'h3' || tag === 'h4') {
      return stripFandomApiArtefacts(el.text().trim());
    }
    el = el.prev();
  }
  return null;
...
async function main() {
  console.log('Fetching 2000 mainline page…');
  ...
  if (!mainlineCollection) {
    throw new Error('2000 Mainline collection not found. Run import_2000_mainline.ts first.');
  }
  ...
  for (let tableIndex = 0; tableIndex < tablesArray.length; tableIndex++) {
    ...
    const tableHeading = findPrevTableHeading(table, $);
    const tableHeadingContext = getHeadingContextBeforeTable(table, $);
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (!toyNumber) {
        const bonusCtx = `${tableHeading ?? ''} ${tableHeadingContext} ${seriesCellRaw}`;
        if (/bonus car/i.test(bonusCtx)) {
          const collectorNumberStr = stripFandomApiArtefacts($(cells[1]).text().trim());
          toyNumber = `${TARGET_YEAR}-BC-${slugify(collectorNumberStr || `row${i}`)}-${slugify(castingName)}`.replace(
            /-+/g,
            '-',
          );
        } else {
      ...
      let cleanedSeriesHint = stripFandomApiArtefacts(
        stripThFromSeriesName(normalizeWikiSeriesSpacing(seriesCellRaw))
          ...
          .replace(/\s*New for 2000\s*/gi, '')
          .replace(/\b2000\s+Bonus\s+Cars\b/gi, 'Bonus Cars')
          .trim(),
      );
      if (/bonus car/i.test(seriesCellRaw)) {
        cleanedSeriesHint = 'Bonus Cars';
      }
