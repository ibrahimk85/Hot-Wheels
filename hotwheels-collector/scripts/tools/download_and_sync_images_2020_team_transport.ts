/**
 * Script to download image assets for the 2020 Hot Wheels Team Transport set and
 * associate them with Model and Variant records in your database.
 *
 * This script performs two jobs:
 *   1. Fetches the 2020 Team Transport page from the Hot Wheels Fandom wiki and
 ...
 *      `public/images/hotwheels/2020/team-transport/` folder.
 ...
 *     the 2020 Team Transport variants using the import script.
 ...
 *
 * Usage:
 *   npx ts-node scripts/tools/download_and_sync_images_2020_team_transport.ts
 */
...
const TEAM_TRANSPORT_URL = 'https://hotwheels.fandom.com/wiki/2020_Car_Culture:_Team_Transport';
    ...
    for (const [setKey, setRows] of groupedBySet.entries()) {
      ...
      for (let carIdx = 0; carIdx < carRows.length; carIdx++) {
        ...
        if (!carRow.photoLooseUrl) {
          console.warn(`  Car row ${carIdx + 1} (${carRow.castingName}) has no Photo Loose URL in Column 7`);
          continue;
        }
        ...
        try {
          ...
          console.log(`  Found car variant: ${variant.releaseName} (ID: ${variant.id})`);
          ...
          if (!fs.existsSync(destPath)) {
            ...
            totalDownloaded++;
            console.log(`Downloaded Photo Loose for car ${carRow.castingName} → ${fileName}`);
          }
          ...
          totalAssociated++;
          console.log(`Associated Photo Loose with car variant ${carRow.castingName}`);
        } catch (err) {
