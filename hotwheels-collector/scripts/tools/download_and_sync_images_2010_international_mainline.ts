/**
 * Script to download image assets for the 2010 International Hot Wheels Mainline and
 * associate them with Variant records in your database.
 *
 * This script processes ALL tables from the 2010 International wiki page, where each table
 ...
 *     the 2010 International mainline variants using the import script provided earlier.
...
const MAINLINE_URL = 'https://hotwheels.fandom.com/wiki/List_of_2010_Hot_Wheels_(International)';
...
async function main() {
  console.log('Fetching 2010 International mainline page…');
  ...
  // Find the "Mainline (International)" SubSeries
  const internationalSubSeries = await prisma.subSeries.findFirst({
    where: {
      name: 'Mainline (International)',
      collectionId: mainlineCollection.id,
    },
  });

  if (!internationalSubSeries) {
    throw new Error('Mainline (International) SubSeries not found. Please import data first.');
  }
  ...
  const baseDir = path.join(process.cwd(), 'public', 'images', 'hotwheels', '2010', 'international', 'mainline');
  ...
  for (let tableIndex = 0; tableIndex < tablesArray.length; tableIndex++) {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      // Find variant by Toy# and SubSeries (PRIMARY MATCHING METHOD)
      let variant = await prisma.variant.findFirst({
        where: {
          ...
          model: {
            collectionId: mainlineCollection.id,
            subSeriesId: internationalSubSeries.id,
          },
        },
        include: {
          model: true,
      ...
      const relativePath = path.join('/images', 'hotwheels', '2010', 'international', 'mainline', castingSlug, fileName).replace(/\\/g, '/');
      try {
