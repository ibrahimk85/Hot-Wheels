/**
 * Script to download image assets for the 2026 Hot Wheels Team Transport set and
 * associate them with Model and Variant records in your database.
 *
 * This script performs two jobs:
 *   1. Fetches the 2026 Team Transport page from the Hot Wheels Fandom wiki and
 ...
 *      `public/images/hotwheels/2026/team-transport/` folder.
 ...
 *     the 2026 Team Transport variants using the import script.
 ...
 *   - "Image Not Available" placeholder images are detected and skipped (logged as "No Image").
 *
 * Usage:
 *   npx ts-node scripts/tools/download_and_sync_images_2026_team_transport.ts
 */
...
const TEAM_TRANSPORT_URL = 'https://hotwheels.fandom.com/wiki/2026_Car_Culture:_Team_Transport';
...
function extractSubSeriesName($: cheerio.CheerioAPI, table: any): string {
  ...
  // Clean up sub-series name: remove trailing [] if present (2024/2025/2026 format)
  const cleanedName = (subSeriesName || 'Unknown Series').replace(/\[\]$/, '');
  return cleanedName;
}

/**
 * Check if image URL is "Image Not Available" placeholder and return null if so
 */
function checkImageNotAvailable(imgUrl: string | null, castingName: string, type: 'Transport' | 'Car' | 'Carded'): string | null {
  if (!imgUrl) {
    return null;
  }
  
  // Check for "Image Not Available" placeholder
  if (imgUrl.includes('Image_Not_Available') || imgUrl.includes('Image%20Not%20Available')) {
    console.log(`  No Image available for ${castingName} (${type})`);
    return null;
  }
...
async function main() {
  ...
  for (let tableIdx = 0; tableIdx < tables.length; tableIdx++) {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      // Transport row pattern: First cell is Toy# (short alphanumeric), second is Series# (digit or #digit or N/A for Supreme Exclusive/Walmart/Fast & Furious)
      // 2024/2025/2026 format: Series# can be "#86" (with # prefix)
      const seriesNumberClean = secondCell.replace(/^#/, ''); // Remove # prefix if present
      ...
      if (firstCell.length >= 3 && firstCell.length <= 8 && /^[A-Z0-9]+$/i.test(firstCell) && !firstCell.includes(' ') &&
        ...
        // For normal Series#, use cleaned version (without # prefix) for consistency
        ...
        if (imgs.length > 0) {
          ...
          if (imgUrl) {
            ...
            const cleanedUrl = imgUrl
              ...
              .replace(/\/revision\/latest\/scale-to-width-down\/\d+/g, '');
            // Check for "Image Not Available" placeholder
            photoLooseUrl = checkImageNotAvailable(cleanedUrl, castingNameRaw, isTransport ? 'Transport' : 'Car');
            if (photoLooseUrl) {
          ...
          if (imgs.length > 0) {
            ...
            imgs.each((_, img) => {
              ...
              if (imgUrl) {
                ...
                // Check for "Image Not Available" placeholder
                const checkedUrl = checkImageNotAvailable(fullImgUrl, castingNameRaw, 'Car');
                
                // Take the first loose image found (if not "Image Not Available")
                if (!photoLooseUrl && checkedUrl) {
                  photoLooseUrl = checkedUrl;
                  foundLooseImage = true;
