/**
 * Fix Boulevard duplicate models for 2021
 * 
 * Strategy:
...
async function main() {
  console.log(`=== BOULEVARD ${targetYear} DUPLICATE FIX ===\n`);
  ...
  // Handle duplicates - keep the one with Mix 1-4 subSeries
  if (duplicates.length > 0) {
    console.log('\n=== RESOLVING DUPLICATES ===\n');
    
    for (const [castingName, models] of duplicates) {
      // Find models with Mix 1-4 subSeries (these are the correct ones)
      const mixModels = models.filter(m => {
        const subSeriesName = m.subSeries?.name || '';
        return /^Mix\s+[1-4]$/i.test(subSeriesName);
      });
      
      // Find models with casting name as subSeries (these are wrong and should be deleted)
      const wrongModels = models.filter(m => {
        const subSeriesName = m.subSeries?.name || '';
        return !/^Mix\s+[1-4]$/i.test(subSeriesName) && subSeriesName !== '';
      });
      
      // Sort mix models: first by hasImages
      const sortedMixModels = mixModels.sort((a, b) => {
      ...
      let keepModel = sortedMixModels[0];
      let deleteModels: typeof models = [];
      
      if (keepModel) {
        deleteModels = [...sortedMixModels.slice(1), ...wrongModels];
      } else {
        // Fallback: if no Mix model found, keep the first one (shouldn't happen)
        console.warn(`  ⚠️  WARNING: No Mix 1-4 model found for ${castingName}, keeping first model`);
        keepModel = models[0];
        deleteModels = models.slice(1);
      }
