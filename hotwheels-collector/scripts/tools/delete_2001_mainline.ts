...
async function delete2001Mainline() {
  console.log('=== Delete 2001 Mainline Data & Images ===\n');
  ...
  console.log(`Models under 2001 Mainline: ${modelIds.length}`);
  ...
  console.log(`Remaining variants for 2001 Mainline: ${remainingVariants}`);
  ...
  if (remainingVariants === 0 && remainingImages === 0 && !folderExistsAfter) {
    console.log('\n✅ 2001 Mainline data and images removed.');
  } else {
    console.log('\n⚠ Some 2001 Mainline data or images may still remain. Review logs.');
  }
}

(async () => {
  try {
    await delete2001Mainline();
  } catch (err) {
    console.error('❌ Error while deleting 2001 Mainline:', err);
    process.exitCode = 1;
  } finally {
