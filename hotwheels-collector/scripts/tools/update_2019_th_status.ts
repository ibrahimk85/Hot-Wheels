/**
 * Script to update 2019 Treasure Hunt and Super Treasure Hunt status
 * by scraping the Hot Wheels Fandom wiki page and matching entries
 * with database variants.
 *
 * This script:
 * 1. Fetches the List of 2019 Hot Wheels page from hotwheels.fandom.com
 ...
 *
 * Usage:
 *   npx ts-node scripts/tools/update_2019_th_status.ts
 */
...
async function updateVariants() {
  console.log('=== 2019 Treasure Hunt Status Update ===\n');

  const variantCount = await prisma.variant.count({
    where: { year: 2019 },
  });
  console.log(`Found ${variantCount} variants in database for year 2019\n`);

  if (variantCount === 0) {
    console.warn('⚠ No 2019 variants found in database. Please import 2019 mainline data first.');
    return;
  }
