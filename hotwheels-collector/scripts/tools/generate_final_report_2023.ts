/**
 * Generate final report for 2023 scraping
 */
...
async function main() {
  ...
  const year2023 = await prisma.year.findFirst({ where: { year: 2023 } });
  if (!year2023) {
    console.log('⚠️  Year 2023 not found. Please run the scraping script first.');
    await prisma.$disconnect();
    return;
  }

  const mainlineCollection = await prisma.collection.findFirst({
    where: { name: 'Mainline', yearId: year2023.id },
    include: {
      _count: {
        select: {
          models: true,
  ...
  if (!mainlineCollection) {
    console.log('⚠️  Mainline collection not found for 2023.');
    await prisma.$disconnect();
    return;
  }
  ...
  const imageDir = path.join(process.cwd(), 'public', 'images', 'hotwheels', '2023', 'mainline');
  ...
  const report = `# 2023 Hot Wheels Mainline Scraping Final Raporu
...
`;
  ...
  const reportPath = path.join(process.cwd(), 'scripts', 'tools', 'SCRAPE_2023_REPORT.md');
