/**
 * Analyze Boulevard duplicate issue
 * 
 * Kullanıcının bildirdiği sorun:
 * - 50 model var ama 25 olmalı
...
async function main() {
  console.log('=== BOULEVARD DUPLICATE ANALYSIS ===\n');
  ...
  console.log(`\n\nExpected final count: 25 models (5 Mix × 5 models)`);
  console.log(`Current count: ${allModels.length} models`);
  console.log(`After cleanup: ~${modelsWithSubSeries.length} models (${modelsWithSubSeries.length - 25} still need investigation)`);
}
