...
async function delete2004Mainline() {
  console.log('=== Delete 2004 Mainline Data & Images ===\n');
  ...
  console.log(`Models under 2004 Mainline: ${modelIds.length}`);
  ...
  console.log(`Remaining variants for 2004 Mainline: ${remainingVariants}`);
  ...
  if (remainingVariants === 0 && remainingImages === 0 && !folderExistsAfter) {
    console.log('\n✅ 2004 Mainline data and images removed.');
  } else {
    console.log('\n⚠ Some 2004 Mainline data or images may still remain. Review logs.');
  }
}

(async () => {
  try {
    await delete2004Mainline();
  } catch (err) {
    console.error('❌ Error while deleting 2004 Mainline:', err);
    process.exitCode = 1;
  } finally {
