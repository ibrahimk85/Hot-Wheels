/**
 * Download images for Silver Series - Vintage: Vintage Club (2026), National Icons (2025),
 * Salt Flat Racers (2025), Vintage Racing Club (2024)
 * npx ts-node scripts/tools/download_and_sync_images_silver_series_vintage.ts
 */
...
const SERIES_CONFIG = [
  { url: 'https://hotwheels.fandom.com/wiki/Vintage_Club_Series_(2026)', year: 2026, seriesName: 'Vintage - Vintage Club (2026)', subdir: 'vintage-club' },
  { url: 'https://hotwheels.fandom.com/wiki/National_Icons_Series_(2025)', year: 2025, seriesName: 'Vintage - National Icons (2025)', subdir: 'national-icons' },
  { url: 'https://hotwheels.fandom.com/wiki/Salt_Flat_Racers_Series_(2025)', year: 2025, seriesName: 'Vintage - Salt Flat Racers (2025)', subdir: 'salt-flat-racers' },
  { url: 'https://hotwheels.fandom.com/wiki/Vintage_Racing_Club_Series_(2024)', year: 2024, seriesName: 'Vintage - Vintage Racing Club (2024)', subdir: 'vintage-racing-club' },
];
...
function parseRow($: cheerio.CheerioAPI, cells: cheerio.Cheerio<any>) {
  ...
  return { toyNumber: toyNumber || colNumber || '', colNumber, castingName, color };
}

function getImageIndices(n: number) {
...
async function processSeries(config: typeof SERIES_CONFIG[0]) {
  const { url, year: targetYear, seriesName, subdir } = config;
  console.log(`\n=== ${seriesName} ===`);
  const res = await fetch(url, { headers: { 'User-Agent': 'Mozilla/5.0' } });
  if (!res.ok) {
  ...
  const BASE_SUBDIR = `silver-series/vintage/${subdir}`;
  ...
  for (let ti = 0; ti < tables.length; ti++) {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const idxs = getImageIndices(cells.length);
      if (!idxs || cells.length < 6) continue;

      const { toyNumber, colNumber, castingName, color } = parseRow($, cells);
      if (!castingName || /^spectraflame|^chase$/i.test(castingName.trim())) continue;
      const toyNum = toyNumber || colNumber || '';
      if (!toyNum && !castingName) continue;
