...
async function delete2006Mainline() {
  console.log('=== Delete 2006 Mainline Data & Images ===\n');
  ...
  console.log(`Models under 2006 Mainline: ${modelIds.length}`);
  ...
  console.log(`Remaining variants for 2006 Mainline: ${remainingVariants}`);
  ...
  if (remainingVariants === 0 && remainingImages === 0 && !folderExistsAfter) {
    console.log('\n✅ 2006 Mainline data and images removed.');
  } else {
    console.log('\n⚠ Some 2006 Mainline data or images may still remain. Review logs.');
  }
}

(async () => {
  try {
    await delete2006Mainline();
  } catch (err) {
    console.error('❌ Error while deleting 2006 Mainline:', err);
    process.exitCode = 1;
  } finally {
