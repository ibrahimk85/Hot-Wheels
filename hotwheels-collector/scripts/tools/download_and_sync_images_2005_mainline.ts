/**
 * Download 2005 Mainline images from Fandom and attach to Variant rows by Toy#.
 *
 * Skips Walmart Redline Exclusives and Goodyear Tire Exclusives tables if present (same rules as import).
 *
 *   npx ts-node scripts/tools/download_and_sync_images_2005_mainline.ts
 *
 * Wiki HTML fallback: set FANDOM_WIKI_HTML_PATH to a saved List_of_2005 page (same as import).
 */
...
const MAINLINE_URL = 'https://hotwheels.fandom.com/wiki/List_of_2005_Hot_Wheels';
...
async function main() {
  console.log('Fetching 2005 mainline page…');
  ...
  if (!mainlineCollection) {
    throw new Error('2005 Mainline collection not found. Run import_2005_mainline.ts first.');
  }
