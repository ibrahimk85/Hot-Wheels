...
async function delete2019PopCulture() {
  console.log('=== Delete 2019 Pop Culture Data & Images ===\n');

  // 1) Find 2019 Pop Culture collection
  ...
  console.log(`Models under 2019 Pop Culture: ${modelIds.length}`);

  // 3) Delete variants (2019 + models in this collection)
  ...
  // 4) Delete images related to 2019 Pop Culture
  const deleteImagesResult = await prisma.image.deleteMany({
    where: {
      OR: [
        // Images stored under the 2019 pop-culture path
        {
          path: {
            startsWith: '/images/hotwheels/2019/pop-culture/',
          },
        },
  ...
  // 8) Filesystem cleanup for 2019 pop-culture images
  const popCultureDir = path.join(process.cwd(), 'public', 'images', 'hotwheels', '2019', 'pop-culture');
  ...
  const popCultureDirPath = path.join(process.cwd(), 'public', 'images', 'hotwheels', '2019', 'pop-culture');
  const folderExistsAfter = fs.existsSync(popCultureDirPath);

  console.log(`Remaining variants for 2019 Pop Culture: ${remainingVariants}`);
  console.log(`Remaining images under /images/hotwheels/2019/pop-culture/: ${remainingImages}`);
  ...
  if (remainingVariants === 0 && remainingImages === 0 && !folderExistsAfter) {
    console.log('\n✅ 2019 Pop Culture data and images successfully removed.');
  } else {
    console.log('\n⚠ Some 2019 Pop Culture data or images may still remain. Please review the logs.');
  }
}

delete2019PopCulture()
  .catch((err) => {
    console.error('❌ Error while deleting 2019 Pop Culture:', err);
    process.exit(1);
  })
  .finally(async () => {
