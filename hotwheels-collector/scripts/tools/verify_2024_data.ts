/**
 * Verification script to check 2024 data
 */
...
async function main() {
  console.log('🔍 Verifying 2024 Mainline Data...\n');

  // Check Year
  const year2024 = await prisma.year.findFirst({ where: { year: 2024 } });
  console.log(`Year 2024: ${year2024 ? '✅ Exists' : '❌ Not found'}`);

  if (!year2024) {
    console.log('\n⚠️  Year 2024 not found. The scraping script may not have run yet.');
  ...
  if (!mainlineCollection) {
    console.log('\n⚠️  Mainline collection not found for 2024.');
    await prisma.$disconnect();
    return;
  }
