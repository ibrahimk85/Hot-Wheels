/**
 * Script to download image assets for the 2024 Hot Wheels Mainline and
 * associate them with Variant records in your database.
 *
 * This script performs two jobs:
 *   1. Fetches the 2024 mainline table from the Hot Wheels Fandom wiki and
 ...
 *      `public/images/hotwheels/2024/mainline/{castingSlug}/` folder. The
 ...
 *     the 2024 mainline variants using the import script provided earlier.
 ...
 *
 *   3. Run with ts-node:
 *        npx ts-node scripts/tools/download_and_sync_images_2024_mainline.ts
 */
...
const MAINLINE_URL = 'https://hotwheels.fandom.com/wiki/List_of_2024_Hot_Wheels';
...
async function main() {
  console.log('Fetching 2024 mainline page…');
  ...
  // public/images/hotwheels/2024/mainline/
  const baseDir = path.join(process.cwd(), 'public', 'images', 'hotwheels', '2024', 'mainline');
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    const relativePath = path.join('/images', 'hotwheels', '2024', 'mainline', castingSlug, fileName)
      .replace(/\\/g, '/');
