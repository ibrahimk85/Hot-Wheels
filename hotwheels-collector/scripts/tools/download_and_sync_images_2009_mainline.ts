/**
 * Download 2009 Mainline images from Fandom and attach to Variant rows by Toy#.
 *
 * Skips Walmart Redline Exclusives and Goodyear Tire Exclusives tables (same rules as import).
 *
 *   npx ts-node scripts/tools/download_and_sync_images_2009_mainline.ts
 *
 * Wiki HTML fallback: set FANDOM_WIKI_HTML_PATH to a saved List_of_2009 page (same as import).
 */
...
const MAINLINE_URL = 'https://hotwheels.fandom.com/wiki/List_of_2009_Hot_Wheels';
...
async function main() {
  console.log('Fetching 2009 mainline page…');
  ...
  if (!mainlineCollection) {
    throw new Error('2009 Mainline collection not found. Run import_2009_mainline.ts first.');
  }
