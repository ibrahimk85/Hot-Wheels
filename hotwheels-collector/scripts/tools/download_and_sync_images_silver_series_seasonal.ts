/**
 * Download images for Silver Series - Seasonal (2026): Spring only; Halloween/Winter empty
 * Spring table: 8 cols - Series#|Toy#|Casting|Color|Wheel Type|Notes|Photo Loose|Photo Carded
 * npx ts-node scripts/tools/download_and_sync_images_silver_series_seasonal.ts
 */
...
const SERIES_CONFIG = [
  { url: 'https://hotwheels.fandom.com/wiki/Spring_Series_(2026)', year: 2026, seriesName: 'Seasonal - Spring (2026)', subdir: 'spring', mixName: 'Vehicles' },
];
...
async function processSeries(config: typeof SERIES_CONFIG[0]) {
  const { url, year: targetYear, seriesName, subdir, mixName } = config;
  console.log(`\n=== ${seriesName} ===`);
  const res = await fetch(url, { headers: { 'User-Agent': 'Mozilla/5.0' } });
  if (!res.ok) {
  ...
  const BASE_SUBDIR = `silver-series/seasonal/${subdir}`;
  ...
  // Spring: 8 cols -> loose=6, carded=7
  const looseIdx = 6;
  const cardedIdx = 7;

  for (let ti = 0; ti < tables.length; ti++) {
    const rows = $(tables[ti]).find('tbody tr').filter((_: any, r: any) => $(r).find('td').length >= 8);
    for (let i = 0; i < rows.length; i++) {
      const cells = $(rows[i]).find('td');
      if (cells.length < 8) continue;

      const { toyNumber, colNumber, castingName, color } = parseRow($, cells);
      if (!castingName || !toyNumber) continue;
      ...
      for (const [idx, kind] of [[cardedIdx, 'carded'] as const, [looseIdx, 'loose'] as const]) {
        ...
        const fname = `${toyNumber}_${kind}.${ext}`.replace(/[<>:"/\\|?*]/g, '');
