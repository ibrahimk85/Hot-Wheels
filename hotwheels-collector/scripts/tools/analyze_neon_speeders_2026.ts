/**
 * Script to analyze Neon Speeders 2026 wiki page structure
 */
...
async function main() {
  ...
  console.log(`Analyzing 2026 section in ${wikiUrl}\n`);

  const html = await fetchWithRetry(wikiUrl);
  const $ = cheerio.load(html);

  // Find heading "2026"
  const heading2026 = $('h2, h3, h4').filter((_, el) => {
    const text = $(el).text().trim();
    return text.includes('2026') && !text.includes('2025') && !text.includes('2024') && !text.includes('2023');
  }).first();

  console.log(`Found heading for 2026: "${heading2026.text().trim()}"`);
  console.log(`Heading level: ${heading2026.prop('tagName')}\n`);

  // Find all tables after 2026 heading until next year heading
  const nextYearHeading = heading2026.nextAll('h2, h3, h4').filter((_, el) => {
    const text = $(el).text().trim();
    return /^\d{4}/.test(text) && !text.includes('2026');
  }).first();

  let tablesToCheck;
  if (nextYearHeading.length > 0) {
    tablesToCheck = heading2026.nextUntil(nextYearHeading).filter('table.wikitable');
  } else {
    tablesToCheck = heading2026.nextAll('table.wikitable');
  }

  console.log(`Found ${tablesToCheck.length} table(s) for 2026\n`);
