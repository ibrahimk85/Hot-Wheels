/**
 * Download Fast & Furious Premium wiki images for 2025 (403-safe fetch + CDN binary download).
 * Skips Bundle 1–3 tables (same as import).
 *
 *   npx ts-node scripts/tools/download_and_sync_images_2025_fast_furious_premium.ts
 */
...
const BUNDLE_SKIP =
  /fast.*furious.*premium.*bundle.*1|fast.*furious.*premium.*bundle.*2|fast.*furious.*premium.*bundle.*3|bundle.*1|bundle.*2|bundle.*3/i;
...
async function main() {
  ...
  const yearTables = allTables.filter((_, table) => {
    ...
    if (BUNDLE_SKIP.test(headingText)) return false;
    return true;
  });
  ...
  for (let tableIdx = 0; tableIdx < yearTables.length; tableIdx++) {
    ...
    if (
      /^(contents|references|see also|external links|categories)$/i.test(subSeriesName) ||
      BUNDLE_SKIP.test(subSeriesName)
    ) {
