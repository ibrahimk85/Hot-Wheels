/**
 * Script to download image assets for the 2021 Hot Wheels Boulevard series.
 * 
 * This script:
 *   1. Fetches the Boulevard table from the Hot Wheels Fandom wiki
 *   2. Extracts Photo Carded and Photo Loose image URLs
 *   3. Downloads images to public/images/hotwheels/2021/boulevard/{castingSlug}/
 ...
 * 
 * How to use:
 *   npx ts-node scripts/tools/download_and_sync_images_2021_boulevard.ts
 */
...
async function main() {
  ...
  for (let tableIdx = 0; tableIdx < tables.length; tableIdx++) {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (!model) {
        // Try to find model without exact casting name match (case-insensitive, trim)
        const alternativeModel = await prisma.model.findFirst({
          where: {
            subSeries: {
              name: mixName,
              collection: {
        ...
        if (alternativeModel) {
          console.warn(`Model not found with exact name "${castingName}" (${mixName}), but found model "${alternativeModel.castingName}"`);
        } else {
