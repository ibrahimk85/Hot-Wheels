/**
 * Script to download Elite 64 Future images only
 * 
 * This script:
 * 1. Reads existing Elite 64 Future models from database
 ...
 * 
 * Usage:
 *   npx ts-node scripts/tools/download_elite64_images_future.ts
 */
...
async function main() {
  console.log('=== Elite 64 Future Image Download Script ===\n');

  // Get Elite 64 Future collection
  const yearRecord = await prisma.year.findFirst({ where: { year: TARGET_YEAR } });
  if (!yearRecord) {
    console.log(`Year ${TARGET_YEAR} (Future) not found. Please run import script first.`);
    return;
  }
  ...
  if (!collection || collection.models.length === 0) {
    console.log(`No Elite 64 Future models found. Please run import script first.`);
    return;
  }

  console.log(`Found ${collection.models.length} models for Elite 64 Future\n`);
  ...
  if (!yearHeader) {
    console.log(`Future section not found on wiki page`);
    return;
  }
  ...
  if (!table || table.length === 0) {
    console.log(`No table found for Future section`);
    return;
  }
  ...
  console.log('\nElite 64 Future image download completed!');
}
