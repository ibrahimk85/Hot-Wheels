/**
 * Script to update 2021 Treasure Hunt and Super Treasure Hunt status
 * by scraping the Hot Wheels Fandom wiki page and matching entries
 * with database variants.
 *
 * This script:
 * 1. Fetches the List of 2021 Hot Wheels page from hotwheels.fandom.com
 ...
 *
 * Usage:
 *   npx ts-node scripts/tools/update_2021_th_status.ts
 */
...
async function parseTreasureHunts(): Promise<Entry[]> {
  ...
  (mainTable as ReturnType<typeof $>).find('tr').slice(1).each((_: any, row: any) => {
    ...
    const seriesInfoText = $(cells[4]).text().trim(); // Series# column (5th column, index 4)

    // Check both Series and Series# columns for TH/STH
...
async function updateVariants() {
  console.log('=== 2021 Treasure Hunt Status Update ===\n');

  // First, check if we have any 2021 variants in the database
  const variantCount = await prisma.variant.count({
    where: { year: 2021 },
  });
  console.log(`Found ${variantCount} variants in database for year 2021\n`);

  if (variantCount === 0) {
    console.warn('⚠ No 2021 variants found in database. Please import 2021 mainline data first.');
    return;
  }
  ...
  // First, reset all TH/STH flags for 2021 Mainline variants to false
  ...
  if (mainlineCollection) {
    const resetCount = await prisma.variant.updateMany({
      where: {
        year: 2021,
        model: {
    ...
    console.log(`Reset ${resetCount.count} variants' TH/STH flags to false\n`);
  }
  ...
  for (const entry of entries) {
    ...
    // For TH: Try to match with castingId (toyNumber) first
    // For STH: STH models may have different castingId, so we'll match by collector number only
    
    let variants: any[] = [];

    if (!isSuper) {
      // Strategy 1: For TH, try exact match with castingId and full collectorNumber (e.g., "017/250")
      ...
      // For STH: Match by collector number only (STH models may have different castingId)
      // Strategy 1: Exact match with full collectorNumber (e.g., "017/250")
      variants = await prisma.variant.findMany({
        where: {
          year: 2021,
          cardNumber: collectorNumber,
