/**
 * Script to download image assets for the 2019 Hot Wheels Mainline and
 * associate them with Variant records in your database.
 *
 * This script performs two jobs:
 *   1. Fetches the 2019 mainline table from the Hot Wheels Fandom wiki and
 ...
 *      `public/images/hotwheels/2019/mainline/{castingSlug}/` folder. The
 ...
 *     the 2019 mainline variants using the import script provided earlier.
 ...
 *
 *   3. Run with ts-node:
 *        npx ts-node scripts/tools/download_and_sync_images_2019_mainline.ts
 */
...
const MAINLINE_URL = 'https://hotwheels.fandom.com/wiki/List_of_2019_Hot_Wheels';
...
async function main() {
  console.log('Fetching 2019 mainline page…');
  ...
  // public/images/hotwheels/2019/mainline/
  const baseDir = path.join(process.cwd(), 'public', 'images', 'hotwheels', '2019', 'mainline');
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (!mainlineCollection) {
      console.warn('2019 Mainline collection not found; skipping image association.');
      continue;
    }
    ...
    // This works even if model name parsing differs between import and image scripts
    ...
    if (!model) {
      ...
      if (baseCastingName !== castingName) {
        model = await prisma.model.findFirst({
          where: {
    ...
    const relativePath = path.join('/images', 'hotwheels', '2019', 'mainline', castingSlug, fileName);
    try {
