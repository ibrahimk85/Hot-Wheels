/**
 * Script to download image assets for the 2014 Hot Wheels Fast & Furious Series.
 * 
 * This script:
 *   1. Fetches the Fast & Furious Series page from the Hot Wheels Fandom wiki
 *   2. Extracts Photo Carded and Photo Loose image URLs for 2014 year table
 *   3. Downloads images to public/images/hotwheels/2014/fast-and-furious/{castingSlug}/
 *   4. Associates images with Variant records
 * 
 * Fast & Furious Series-specific (2014 ONLY):
 * - Photo - Car (Loose) → Main image (variant.imageId) - Column 8
 * - Photo Carded → Second image (variant.images[]) - Column 7
 * NOT: 2014'te tabloda "Photo - Car" yazıyor, bu Photo Loose anlamına geliyor ve ANA RESİM olarak kullanılıyor
 * - File names: {toyNumber}_carded.jpg and {toyNumber}_loose.jpg
 * - Variant matching: Year + Series # (Col #) + Casting Name + Color + SubSeries
 * 
 * 2014 Table Structure (9 columns):
 * - Column 0: Col # (Series #) - e.g., "1/8"
 * - Column 1: Toy #
 * - Column 2: Casting Name (link)
 * - Column 3: Color
 * - Column 4: Tampo
 * - Column 5: Wheel Type
 * - Column 6: Base code(s)
 * - Column 7: Photo Carded (ikinci resim)
 * - Column 8: Photo - Car (Loose, ana resim) - NOT: Tabloda "Photo - Car" yazıyor, bu Photo Loose anlamına geliyor ve ANA RESİM olarak kullanılıyor
 * 
 * How to use:
 *   npx ts-node scripts/tools/download_and_sync_images_2014_fast_and_furious.ts
 */
...
async function main() {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    // Process Photo - Car (Loose, main image) - Column 8
    // NOT: 2014'te tabloda "Photo - Car" yazıyor, bu Photo Loose anlamına geliyor
    // Column 8 ana resim olarak kullanılıyor
    const photoLooseColIdx = 8;
      ...
      if (photoLooseImgUrlRaw) {
        ...
        if (!fs.existsSync(destPath)) {
          try {
            ...
            downloadCount++;
            console.log(`Downloaded Photo - Car (Loose): ${castingName} → ${fileName}`);
          } catch (err) {
            console.error(`Error downloading Photo - Car (Loose) for ${castingName}:`, err);
          }
        }
        ...
        if (fs.existsSync(destPath)) {
          ...
          try {
            ...
            // Update variant.imageId to point to loose image (main image for 2014)
            await prisma.variant.update({
              where: { id: variant.id },
              data: { imageId: imageRecord.id },
            });
            associatedCount++;
            console.log(`Associated Photo - Car (Loose) as main image with variant ${castingName}`);
          } catch (err) {
            console.error(`Error associating Photo - Car (Loose):`, err);
          }
        }
      }
    }

    // Process Photo Carded (second image) - Column 7
    const photoCardedColIdx = 7;
    if (cells.length > photoCardedColIdx) {
      ...
      if (photoCardedImgUrlRaw) {
        ...
        // Check if carded image already exists in database
        const existingCardedImage = await prisma.image.findFirst({
          where: {
            variantId: variant.id,
            path: {
              contains: sanitizedToyNumber ? `${sanitizedToyNumber}_carded` : `${castingSlug}_carded`,
            },
          },
        });

        // Download if not exists
        if (!fs.existsSync(destPath) && !existingCardedImage) {
          try {
        ...
        if (!existingCardedImage) {
          ...
          if (fs.existsSync(destPath)) {
            try {
              ...
              associatedCount++;
              console.log(`Associated Photo Carded as second image with variant ${castingName}`);
            } catch (err) {
              console.error(`Error associating Photo Carded:`, err);
            }
          }
        } else {
          console.log(`Photo Carded already associated with variant ${castingName}`);
        }
      }
    }
  }
