...
async function delete2023Mainline() {
  console.log('=== Delete 2023 Mainline Data & Images ===\n');

  // 1) Find 2023 Mainline collection
  ...
  console.log(`Models under 2023 Mainline: ${modelIds.length}`);

  // 3) Delete variants (2023 + models in this collection)
  ...
  // 4) Delete images related to 2023 Mainline
  const deleteImagesResult = await prisma.image.deleteMany({
    where: {
      OR: [
        // Images stored under the 2023 mainline path
        {
          path: {
            startsWith: '/images/hotwheels/2023/mainline/',
          },
        },
  ...
  // 6) Filesystem cleanup for 2023 mainline images
  const mainlineDir = path.join(process.cwd(), 'public', 'images', 'hotwheels', '2023', 'mainline');
  ...
  const mainlineDirPath = path.join(process.cwd(), 'public', 'images', 'hotwheels', '2023', 'mainline');
  const folderExistsAfter = fs.existsSync(mainlineDirPath);

  console.log(`Remaining variants for 2023 Mainline: ${remainingVariants}`);
  console.log(`Remaining images under /images/hotwheels/2023/mainline/: ${remainingImages}`);
  ...
  if (remainingVariants === 0 && remainingImages === 0 && !folderExistsAfter) {
    console.log('\n✅ 2023 Mainline data and images successfully removed.');
  } else {
    console.log('\n⚠ Some 2023 Mainline data or images may still remain. Please review the logs.');
  }
}

delete2023Mainline()
  .catch((err) => {
    console.error('❌ Error while deleting 2023 Mainline:', err);
    process.exit(1);
  })
  .finally(async () => {
