/**
 * Download 2001 Mainline images from Fandom and attach to Variant rows by Toy#.
 *
 * Skips Walmart Redline Exclusives and Goodyear Tire Exclusives tables if present (same rules as import).
 *
 *   npx ts-node scripts/tools/download_and_sync_images_2001_mainline.ts
 *
 * Wiki HTML fallback: set FANDOM_WIKI_HTML_PATH to a saved List_of_2001 page (same as import).
 */
...
const MAINLINE_URL = 'https://hotwheels.fandom.com/wiki/List_of_2001_Hot_Wheels';
const TARGET_YEAR = 2001;
...
async function main() {
  console.log('Fetching 2001 mainline page…');
  ...
  if (!mainlineCollection) {
    throw new Error('2001 Mainline collection not found. Run import_2001_mainline.ts first.');
  }
  ...
  for (let tableIndex = 0; tableIndex < tablesArray.length; tableIndex++) {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (!toyNumber) {
        const storeCtx = `${tableHeading ?? ''} ${tableHeadingContext} ${seriesCellRaw}`;
        if (/store exclusive|costco|sam'?s club/i.test(storeCtx)) {
          toyNumber = `${TARGET_YEAR}-SE-${slugify(seriesCellRaw)}-${slugify(castingName)}-${i}`.replace(
            /-+/g,
            '-',
          );
        } else {
