/**
 * Script to download image assets for the 2019 Hot Wheels Pop Culture series.
 * 
 * This script:
 *   1. Fetches the Pop Culture table from the Hot Wheels Fandom wiki
 *   2. Extracts Photo Carded and Photo Loose image URLs
 *   3. Downloads images to public/images/hotwheels/2019/pop-culture/{castingSlug}/
 ...
 * 
 * How to use:
 *   npx ts-node scripts/tools/download_and_sync_images_2019_pop_culture.ts
 */
...
async function main() {
  ...
  for (let tableIdx = 0; tableIdx < tables.length; tableIdx++) {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      // Column 5: Wheel Type
      // Column 6: Notes
      // Column 7: Photo Loose (resim)
      // Column 8: Photo Carded (resim)
      ...
      if (cells.length > 8) {
        const cardedImgElement = $(cells[8]).find('img').first();
      ...
      if (cells.length > 7) {
        const looseImgElement = $(cells[7]).find('img').first();
