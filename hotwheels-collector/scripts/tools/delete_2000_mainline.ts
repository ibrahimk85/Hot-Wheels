...
async function delete2000Mainline() {
  console.log('=== Delete 2000 Mainline Data & Images ===\n');
  ...
  console.log(`Models under 2000 Mainline: ${modelIds.length}`);
  ...
  console.log(`Remaining variants for 2000 Mainline: ${remainingVariants}`);
  ...
  if (remainingVariants === 0 && remainingImages === 0 && !folderExistsAfter) {
    console.log('\n✅ 2000 Mainline data and images removed.');
  } else {
    console.log('\n⚠ Some 2000 Mainline data or images may still remain. Review logs.');
  }
}

(async () => {
  try {
    await delete2000Mainline();
  } catch (err) {
    console.error('❌ Error while deleting 2000 Mainline:', err);
    process.exitCode = 1;
  } finally {
