...
const TARGET_YEAR = 2009;
const MAINLINE_NAME = 'Mainline';

async function delete2009Mainline() {
  console.log('=== Delete 2009 Mainline Data & Images ===\n');
  ...
  console.log(`Models under 2009 Mainline: ${modelIds.length}`);
  ...
  const deleteImagesResult = await prisma.image.deleteMany({
    where: {
      OR: [
        { path: { startsWith: '/images/hotwheels/2009/mainline/' } },
        {
          variant: {
            model: { collectionId: collection.id },
          },
  ...
  const mainlineDir = path.join(process.cwd(), 'public', 'images', 'hotwheels', '2009', 'mainline');
  ...
  const remainingImages = await prisma.image.count({
    where: { path: { startsWith: '/images/hotwheels/2009/mainline/' } },
  });
  const folderExistsAfter = fs.existsSync(mainlineDir);

  console.log(`Remaining variants for 2009 Mainline: ${remainingVariants}`);
  console.log(`Remaining images under /images/hotwheels/2009/mainline/: ${remainingImages}`);
  ...
  if (remainingVariants === 0 && remainingImages === 0 && !folderExistsAfter) {
    console.log('\n✅ 2009 Mainline data and images removed.');
  } else {
    console.log('\n⚠ Some 2009 Mainline data or images may still remain. Review logs.');
  }
}

(async () => {
  try {
    await delete2009Mainline();
  } catch (err) {
    console.error('❌ Error while deleting 2009 Mainline:', err);
    process.exitCode = 1;
  } finally {
