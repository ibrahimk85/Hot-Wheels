/**
 * Download 2006 Mainline images from Fandom and attach to Variant rows by Toy#.
 *
 * Skips Walmart Redline Exclusives and Goodyear Tire Exclusives tables if present (same rules as import).
 *
 *   npx ts-node scripts/tools/download_and_sync_images_2006_mainline.ts
 *
 * Wiki HTML fallback: set FANDOM_WIKI_HTML_PATH to a saved List_of_2006 page (same as import).
 */
...
const MAINLINE_URL = 'https://hotwheels.fandom.com/wiki/List_of_2006_Hot_Wheels';
const TARGET_YEAR = 2006;
...
async function main() {
  console.log('Fetching 2006 mainline page…');
  ...
  if (!mainlineCollection) {
    throw new Error('2006 Mainline collection not found. Run import_2006_mainline.ts first.');
  }
