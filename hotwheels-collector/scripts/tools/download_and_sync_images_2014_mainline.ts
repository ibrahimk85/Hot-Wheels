/**
 * Script to download image assets for the 2014 Hot Wheels Mainline and
 * associate them with Variant records in your database.
 *
 * This script processes ALL tables from the 2014 wiki page, where each table
 ...
 *     the 2014 mainline variants using the import script provided earlier.
 *   - CRITICAL: 2014'de 2nd/3rd color varyantları aynı COL# ama farklı Toy#.
...
const MAINLINE_URL = 'https://hotwheels.fandom.com/wiki/List_of_2014_Hot_Wheels';
...
async function main() {
  console.log('Fetching 2014 mainline page…');
  const html = await fetchFandomWikiHtml(MAINLINE_URL);
  const $ = cheerio.load(html);

  // Find 2014 Mainline collection
  ...
  if (!mainlineCollection) {
    throw new Error('2014 Mainline collection not found. Please import data first.');
  }
  ...
  const baseDir = path.join(process.cwd(), 'public', 'images', 'hotwheels', '2014', 'mainline');
  ...
  for (let tableIndex = 0; tableIndex < tablesArray.length; tableIndex++) {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const relativePath = path.join('/images', 'hotwheels', '2014', 'mainline', castingSlug, fileName).replace(/\\/g, '/');
      try {
