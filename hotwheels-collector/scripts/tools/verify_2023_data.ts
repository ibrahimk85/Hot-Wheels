/**
 * Verification script to check 2023 data
 */
...
async function main() {
  console.log('🔍 Verifying 2023 Mainline Data...\n');

  // Check Year
  const year2023 = await prisma.year.findFirst({ where: { year: 2023 } });
  console.log(`Year 2023: ${year2023 ? '✅ Exists' : '❌ Not found'}`);

  if (!year2023) {
    console.log('\n⚠️  Year 2023 not found. The scraping script may not have run yet.');
  ...
  if (!mainlineCollection) {
    console.log('\n⚠️  Mainline collection not found for 2023.');
    await prisma.$disconnect();
    return;
  }
