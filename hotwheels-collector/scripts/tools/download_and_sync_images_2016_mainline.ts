/**
 * Script to download image assets for the 2016 Hot Wheels Mainline and
 * associate them with Variant records in your database.
 *
 * This script processes ALL tables from the 2016 wiki page, where each table
 ...
 *     the 2016 mainline variants using the import script provided earlier.
 *   - CRITICAL: 2016'da 2nd/3rd color varyantları aynı COL# ama farklı Toy#.
...
const MAINLINE_URL = 'https://hotwheels.fandom.com/wiki/List_of_2016_Hot_Wheels';
...
async function main() {
  console.log('Fetching 2016 mainline page…');
  ...
  if (!mainlineCollection) {
    throw new Error('2016 Mainline collection not found. Please import data first.');
  }
  ...
  const baseDir = path.join(process.cwd(), 'public', 'images', 'hotwheels', '2016', 'mainline');
  ...
  allTables.each((index, tableElement) => {
    ...
    if (rows.length < 2) {
      return; // Skip tables with too few rows
    }
    
    console.log(`\nProcessing table ${index + 1} (${rows.length} rows)...`);
  });
  ...
  for (let tableIndex = 0; tableIndex < tablesArray.length; tableIndex++) {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const relativePath = path.join('/images', 'hotwheels', '2016', 'mainline', castingSlug, fileName).replace(/\\/g, '/');
      try {
