/**
 * Download images for 2024 Motorcycle Club (Automotive) - 10-col table
 * npx ts-node scripts/tools/download_and_sync_images_2024_silver_series_motorcycle_club.ts
 */
...
const WIKI_URL = 'https://hotwheels.fandom.com/wiki/Hot_Wheels_Motorcycle_Club_Series_(2024)';
const COLLECTION_NAME = 'Hot Wheels Silver Series';
const SERIES_NAME = 'Motorcycle Club (2024)';
const BASE_SUBDIR = 'silver-series/motorcycle-club';
...
async function main() {
  console.log('=== MOTORCYCLE CLUB (2024) IMAGE SCRIPT ===');
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      for (const [idx, kind] of [[cardedIdx, 'carded'] as const, [looseIdx, 'loose'] as const]) {
        ...
        if (!fs.existsSync(destPath)) {
          try { await download(url, destPath); dl++; console.log(`Downloaded ${kind}: ${castingName} → ${fname}`); } catch (e) { console.error(e); }
        }
