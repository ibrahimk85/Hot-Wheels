/**
 * Generate final report for 2024 scraping
 */
...
async function main() {
  ...
  const year2024 = await prisma.year.findFirst({ where: { year: 2024 } });
  if (!year2024) {
    console.log('⚠️  Year 2024 not found. Please run the scraping script first.');
    await prisma.$disconnect();
    return;
  }

  const mainlineCollection = await prisma.collection.findFirst({
    where: { name: 'Mainline', yearId: year2024.id },
    include: {
      _count: {
        select: {
          models: true,
  ...
  if (!mainlineCollection) {
    console.log('⚠️  Mainline collection not found for 2024.');
    await prisma.$disconnect();
    return;
  }
  ...
  const imageDir = path.join(process.cwd(), 'public', 'images', 'hotwheels', '2024', 'mainline');
  ...
  const report = `# 2024 Hot Wheels Mainline Scraping Final Raporu
...
`;
  ...
  const reportPath = path.join(process.cwd(), 'scripts', 'tools', 'SCRAPE_2024_REPORT.md');
