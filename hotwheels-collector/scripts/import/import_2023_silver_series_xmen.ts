/**
 * Import 2025 Hot Wheels Silver Series - X-Men (Entertainment category)
 * Supports 8-col and 10-col tables
 * npx ts-node scripts/import/import_2023_silver_series_xmen.ts
 */
...
const URL = 'https://hotwheels.fandom.com/wiki/X-Men_Series_(2023)';
...
async function main() {
  console.log(`Fetching X-Men (${targetYear}) from ${URL}…`);
  const res = await fetch(URL, { headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' } });
