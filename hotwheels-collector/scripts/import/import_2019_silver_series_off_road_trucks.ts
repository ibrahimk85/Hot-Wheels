/**
 * Import 2019 Hot Wheels Silver Series - Off Road Trucks (Automotive category)
 * 11-col: No. | Toy # | Casting | Color | Tampo | Base | Window | Interior | Wheel Type | Notes | Photo | Photo Card
 * npx ts-node scripts/import/import_2019_silver_series_off_road_trucks.ts
 */
...
const URL = 'https://hotwheels.fandom.com/wiki/Off_Road_Trucks_Series_(2019)';
...
function parseRow($: cheerio.CheerioAPI, cells: cheerio.Cheerio<any>) {
  ...
  const wheelType = cells.length > 7 ? $(cells[7]).text().trim() : '';
...
async function main() {
  console.log(`Fetching Off Road Trucks (${targetYear}) from ${URL}…`);
  const res = await fetch(URL, { headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' } });
