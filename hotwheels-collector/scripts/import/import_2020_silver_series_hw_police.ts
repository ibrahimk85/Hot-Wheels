/**
 * Import 2020 Hot Wheels Silver Series - HW Police (Automotive category)
 * 10-col: Col # | Toy # | Casting | Color | Tampos | Wheel Type | Notes | Photo Loose | Photo Carded Missing Logo | Photo Carded Returned
 * npx ts-node scripts/import/import_2020_silver_series_hw_police.ts
 */
...
const URL = 'https://hotwheels.fandom.com/wiki/HW_Police_Series_(2020)';
...
async function main() {
  console.log(`Fetching HW Police (${targetYear}) from ${URL}…`);
  const res = await fetch(URL, { headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' } });
