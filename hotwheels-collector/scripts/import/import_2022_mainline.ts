/**
 * Script to import the 2022 Hot Wheels Mainline set into your database using Prisma.
 *
 * This script fetches the 2022 mainline table from the Hot Wheels Fandom wiki,
 * parses each row to extract the toy number, collector number, model name,
 * sub‑series, series info (e.g. New for 2022!, Treasure Hunt, Super Treasure Hunt,
 ...
 *      setup, the 2022 mainline data will be associated with the `Year`
 *      record for 2022 and the `Collection` record named "Mainline".
 *
 *   3. Run the script with ts-node:
 *
 *         npx ts-node import_2022_mainline.ts
 *
 *      Alternatively, if you have added a script entry in your package.json
 *      (e.g. "import:2022-mainline": "ts-node import_2022_mainline.ts"), you can
 *      run it via `npm run import:2022-mainline`.
 *
 * Notes:
...
// URL of the Fandom page containing the 2022 mainline list
...
async function main() {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    const isNewFor2022 = /New for 2022/i.test(seriesInfoRaw) || /New for 2022/i.test(subSeriesNameRaw);
    ...
    if (isNewFor2022) notesParts.push('New for 2022');
