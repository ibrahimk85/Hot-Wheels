/**
 * Import 2021 Hot Wheels Silver Series - 1/4 Mile Finals (Automotive category)
 * 12-col: Series # | Casting Name | Toy # | Color | Tampo | Base | Window | Interior | Wheel Type | Notes | Photo Loose | Photo Card
 * npx ts-node scripts/import/import_2021_silver_series_quarter_mile_finals.ts
 */
...
const URL = 'https://hotwheels.fandom.com/wiki/1/4_Mile_Finals_Series_(2021)';
...
async function main() {
  console.log(`Fetching 1/4 Mile Finals (${targetYear}) from ${URL}…`);
  const res = await fetch(URL, { headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' } });
