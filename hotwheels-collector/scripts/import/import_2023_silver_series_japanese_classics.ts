/**
 * Import 2023 Hot Wheels Silver Series - Japanese Classics (Automotive category)
 * Table: Series # | Toy # | Casting Name | Color | Tampos | Wheel Type | Notes | Photo Loose | Photo Carded (8 cols)
 * npx ts-node scripts/import/import_2023_silver_series_japanese_classics.ts
 */
...
const URL = 'https://hotwheels.fandom.com/wiki/Japanese_Classics_Series_(2023)';
const COLLECTION_NAME = 'Hot Wheels Silver Series';
const SERIES_NAME = 'Japanese Classics (2023)';
...
/** 8-col: Series # | Toy # | Casting Name | Color | Tampos | Wheel Type | Notes | Photo Loose | Photo Carded */
...
async function main() {
  console.log(`Fetching Japanese Classics (${targetYear}) from ${URL}…`);
  const res = await fetch(URL, { headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' } });
