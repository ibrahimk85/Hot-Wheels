/**
 * Import 2023 Hot Wheels Silver Series - Mud Runners (Automotive category)
 * Table: Series # | Toy # | Casting Name | Color | Tampos | Wheel Type | Notes | Photo Loose | Photo Carded (8 cols)
 * npx ts-node scripts/import/import_2023_silver_series_mud_runners.ts
 */
...
const URL = 'https://hotwheels.fandom.com/wiki/Mud_Runners_Series_(2023)';
const COLLECTION_NAME = 'Hot Wheels Silver Series';
const SERIES_NAME = 'Mud Runners (2023)';
...
async function main() {
  console.log(`Fetching Mud Runners (${targetYear}) from ${URL}…`);
  const res = await fetch(URL, { headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' } });
