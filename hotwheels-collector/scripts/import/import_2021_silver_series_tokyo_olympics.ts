...
const URL = 'https://hotwheels.fandom.com/wiki/Tokyo_2020_Olympics_Series_(2021)';
...
// Tokyo Olympics: Series # | Casting | Toy # | Color | Sport | Tampo | Base | Window | Interior | Wheel Type | Notes | Photo | Photo Card
function parseRow($: cheerio.CheerioAPI, cells: cheerio.Cheerio<any>) {
  ...
  const toyNumber = isTokyo && cells.length > 2 ? $(cells[2]).text().trim() : (cells.length > 1 ? $(cells[1]).text().trim() : '');
  ...
  const wheelType = isTokyo ? (cells.length > 9 ? $(cells[9]).text().trim() : '') : (cells.length > 5 ? $(cells[5]).text().trim() : '');
  const notes = isTokyo ? (cells.length > 10 ? $(cells[10]).text().trim() : '') : (cells.length > 6 ? $(cells[6]).text().trim() : '');
  return { colNumber, toyNumber, castingName, color, wheelType, notes, castingNameLink: link };
}

async function main() {
  console.log(`Fetching Tokyo 2020 Olympics (${targetYear}) from ${URL}…`);
  const res = await fetch(URL, { headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' } });
