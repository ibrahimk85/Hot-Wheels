/**
 * Script to import the 2004 Hot Wheels Mainline set into your database using Prisma.
 *
 * Fetches ALL mainline tables from the 2004 Fandom wiki page (multi-table layout):
 * 2004 First Editions, T-Hunt (Treasure Hunt), Segment Series (Cereal Crunchers, Tat Rods,
 * Star Spangled 2, Ferrari Heat, Final Run, Tag Rides, Crank Itz, Demonition, Scrapheads,
 * Autonomicals, Pride Rides, Roll Patrol, Track Aces, Wastelanders), etc.
 * Skips Walmart "Redline Exclusives" and "Goodyear Tire Exclusives" if present (same rules as 2005+).
 *
 ...
 * 2004 wiki uses "T-Hunt" in headings; combinedText includes heading context for TH/STH flags.
 *
 *   npx ts-node scripts/import/import_2004_mainline.ts
 *
...
const URL = 'https://hotwheels.fandom.com/wiki/List_of_2004_Hot_Wheels';
...
async function main() {
  console.log('Fetching 2004 mainline data…');
  ...
  for (const tableInfo of tablesToProcess) {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const isNewFor2004 = /New for 2004/i.test(combinedText);
      ...
      if (isNewFor2004) notesParts.push('New for 2004');
