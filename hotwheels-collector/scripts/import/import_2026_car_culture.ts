/**
 * Script to import the 2026 Hot Wheels Car Culture set into your database using Prisma.
 *
 * This script:
 *   1. Fetches the 2026 Car Culture page from Hot Wheels Fandom wiki
 ...
 *   5. Creates database records: Year → Collection (Car Culture) → SubSeries → Model → Variant
 *
 * Car Culture-specific:
 * - No TH/STH (always false)
 * - SubSeries names are extracted from headings before tables
 * - Model metadata from detail pages
 * - Skips Team Transport table
 *
 * How to use:
 *   npx ts-node scripts/import/import_2026_car_culture.ts
 */
...
const URL = 'https://hotwheels.fandom.com/wiki/2026_Car_Culture';
...
/**
 * Parse row - handle both Toy#|Series# and Col#|Toy# column orders
 */
function parseRow($: cheerio.CheerioAPI, cells: cheerio.Cheerio<any>): {
  ...
  const toyNumber = isCollectorFirst ? cell1 : cell0;
  ...
  return { toyNumber, seriesNumber, castingName, bodyColor, wheelType, notes, castingNameLink };
}

async function main() {
  console.log(`Fetching ${targetYear} Car Culture data from ${URL}…`);
  ...
  const collectionName = 'Car Culture';
  ...
  for (let tableIdx = 0; tableIdx < tables.length; tableIdx++) {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (!parsed.toyNumber || !parsed.seriesNumber || !parsed.castingName) {
        console.warn(`Skipping row: Toy#=${parsed.toyNumber}, Series#=${parsed.seriesNumber}, Name=${parsed.castingName}`);
        continue;
      }

      const { toyNumber, seriesNumber, castingName, bodyColor, wheelType, notes, castingNameLink } = parsed;
