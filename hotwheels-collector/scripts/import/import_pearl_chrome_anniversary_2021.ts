/**
 * Script to import the 2021 Pearl&Chrome Anniversary Series (Orange and Blue Series)
 * into your database using Prisma.
 *
 * This year has Mix 1 and Mix 2 sub-series.
 *
 * How to use:
 *   npx ts-node scripts/import/import_pearl_chrome_anniversary_2021.ts
 */
...
const SUB_SERIES_NAME = 'Orange and Blue Series';
...
async function main() {
  ...
  // Find Mix headings (Mix 1, Mix 2)
  const mixHeadings = $('h2, h3, h4').filter((_, el) => {
    const text = $(el).text().trim();
    return /Mix\s+[12]/i.test(text);
  });

  let tablesToProcess: Array<{ table: cheerio.Cheerio<cheerio.Element>; mixNumber: string | null; subSeriesName: string }> = [];
  ...
  if (mixHeadings.length > 0) {
    ...
    mixHeadings.each((_, heading) => {
      ...
      const nextHeading = $(heading).nextAll('h2, h3, h4').filter((_, el) => {
        const text = $(el).text().trim();
        return /Mix\s+[12]/i.test(text);
      }).first();
      ...
      if (table.length > 0) {
        tablesToProcess.push({ 
          table, 
          mixNumber, 
          subSeriesName: `${SUB_SERIES_NAME} - Mix ${mixNumber}` 
        });
      }
    });
  } else {
    ...
    if (table.length > 0) {
      tablesToProcess.push({ table, mixNumber: null, subSeriesName: SUB_SERIES_NAME });
    }
  }
