/**
 * Import 2024 Hot Wheels Silver Series - Hot Wagons (Automotive category)
 * Table: Series # | Toy # | Casting Name | Color | Tampos | Wheel Type | Notes | Photo Loose | Photo Carded (8 cols)
 * npx ts-node scripts/import/import_2024_silver_series_hot_wagons.ts
 */
...
const URL = 'https://hotwheels.fandom.com/wiki/Hot_Wagons_Series_(2024)';
...
async function main() {
  console.log(`Fetching Hot Wagons (${targetYear}) from ${URL}…`);
  const res = await fetch(URL, { headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' } });
