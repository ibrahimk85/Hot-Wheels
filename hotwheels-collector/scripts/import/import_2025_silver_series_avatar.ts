/**
 * Import 2025 Hot Wheels Silver Series - Avatar: The Last Airbender (Entertainment category)
 * Supports 8-col and 10-col tables
 * npx ts-node scripts/import/import_2025_silver_series_avatar.ts
 */
...
const URL = 'https://hotwheels.fandom.com/wiki/Avatar:_The_Last_Airbender_Series_(2025)';
...
async function main() {
  console.log(`Fetching Avatar: The Last Airbender (${targetYear}) from ${URL}…`);
  const res = await fetch(URL, { headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' } });
