/**
 * Import 2021 Hot Wheels Silver Series - Cult Racers (Automotive category)
 * 8-col: Series # | Toy # | Casting Name | Color | Tampos | Wheel Type | Notes | Photo Loose | Photo Carded
 * npx ts-node scripts/import/import_2021_silver_series_cult_racers.ts
 */
...
const URL = 'https://hotwheels.fandom.com/wiki/Cult_Racers_Series_(2021)';
const COLLECTION_NAME = 'Hot Wheels Silver Series';
const SERIES_NAME = 'Cult Racers (2021)';
...
async function main() {
  console.log(`Fetching Cult Racers (${targetYear}) from ${URL}…`);
  const res = await fetch(URL, { headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' } });
