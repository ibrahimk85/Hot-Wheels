/**
 * Import 2020 Hot Wheels Silver Series - Forza Motorsport (Automotive category)
 * 8-col: Series # | Toy # | Casting | Color | Tampos | Wheel Type | Notes | Photo - Car | Photo - Card
 * npx ts-node scripts/import/import_2020_silver_series_forza_motorsport.ts
 */
...
const URL = 'https://hotwheels.fandom.com/wiki/Forza_Motorsport_Series_(2020)';
...
async function main() {
  console.log(`Fetching Forza Motorsport (${targetYear}) from ${URL}…`);
  const res = await fetch(URL, { headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' } });
