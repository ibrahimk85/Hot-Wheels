/**
 * Import 2025 Hot Wheels Silver Series - Surf's Up (Automotive category)
 * Table: Series # | Toy # | Casting Name | Body Color | Tampo | Wheel Type | Notes | Photo loose | Photo carded (8 cols)
 * npx ts-node scripts/import/import_2025_silver_series_surfs_up.ts
 */
...
const URL = "https://hotwheels.fandom.com/wiki/Surf%27s_Up_Series_(2025)";
const COLLECTION_NAME = 'Hot Wheels Silver Series';
const SERIES_NAME = "Surf's Up (2025)";
...
async function main() {
  console.log(`Fetching Surf's Up (${targetYear}) from ${URL}…`);
  const res = await fetch(URL, { headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' } });
