/**
 * Script to import the Stars & Stripes Series set into your database using Prisma.
 *
 * This script:
 *   1. Fetches Stars & Stripes Series page from Hot Wheels Fandom wiki
 ...
 *   4. Creates database records: Year → Collection (Stars & Stripes) → SubSeries → Model → Variant
 *
 * Stars & Stripes Series-specific:
 * - No TH/STH (always false)
 * - SubSeries: Year name (2016, 2018, 2020, 2022, 2024)
...
async function processYear(year: number) {
  ...
  // Ensure the Collection record exists for "Stars & Stripes"
  const collectionName = 'Stars & Stripes';
  ...
  for (let tableIndex = 0; tableIndex < tablesToProcess.length; tableIndex++) {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      // Table structure for Stars & Stripes:
      ...
      // Create Variant - Stars & Stripes has NO TH/STH
