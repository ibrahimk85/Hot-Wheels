/**
 * Script to import the 2001 Hot Wheels Mainline set into your database using Prisma.
 *
 * Fetches ALL mainline tables from the 2001 Fandom wiki page (multi-table layout):
 * 2001 Treasure Hunts, 2001 First Editions, Segment Series (Fossil Fuel, Turbo Taxi, Rat Rods, Anime,
 * Rod Squadron, Skull & Crossbones, Logo-Motive, Monsters, Extreme Sports, Company Cars, Hippie Mobiles,
 * Skin Deep), 2001 Hot Wheels, 2001 Final Run, 2001 Store Exclusives (if Toy#/COL# tables).
 * Skips Walmart "Redline Exclusives" and "Goodyear Tire Exclusives" if present (same rules as 2002+).
 *
 ...
 * Final Run → SubSeries "Final Run" (not "2001 Final Run"). Store Exclusives rows often omit Toy# — synthetic `2001-SE-…` keys.
 *
 *   npx ts-node scripts/import/import_2001_mainline.ts
 *
...
const URL = 'https://hotwheels.fandom.com/wiki/List_of_2001_Hot_Wheels';
...
async function main() {
  console.log('Fetching 2001 mainline data…');
  ...
  allTables.each((index, tableElement) => {
    ...
    // e.g. wiki cell "2001s" while h2 is "2001 Treasure Hunts"
    if (/^\d{4}s$/i.test(subSeriesName) && heading && /treasure/i.test(heading)) {
      subSeriesName = stripFandomApiArtefacts(heading);
    }
    if (/\bfinal run\b/i.test(heading ?? '') || /\b2001\s+final run\b/i.test(subSeriesName)) {
      subSeriesName = 'Final Run';
    }
    if (/\bstore\s+exclusive/i.test(heading ?? '') || /\b2001\s+store\s+exclusive/i.test(heading ?? '')) {
      subSeriesName = 'Store Exclusives';
    }
  ...
  for (const tableInfo of tablesToProcess) {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      let toyNumber = $(cells[0]).text().trim();
      if (!toyNumber) {
        const storeCtx = `${heading ?? ''} ${headingContext} ${subSeriesName} ${subSeriesNameRaw}`;
        if (/store exclusive|costco|sam'?s club/i.test(storeCtx)) {
          toyNumber = `${targetYear}-SE-${slugify(subSeriesNameRaw)}-${slugify(castingName)}-${i}`.replace(
            /-+/g,
            '-',
          );
        } else {
      ...
      let cleanedSubSeriesName = stripThFromSeriesName(subSeriesNameRaw)
        ...
        .replace(/\s*New for 2001!\s*/gi, '')
        .replace(/\s*New in Mainline\s*/gi, '')
        .replace(/\s*New for 2001\s*/gi, '')
        .replace(/\s*2001\s+Final Run\s*/gi, '')
        .trim();
      ...
      if (subSeriesName === 'Final Run') {
        finalSubSeriesName = 'Final Run';
      }
      ...
      const isNewFor2001 = /New for 2001/i.test(combinedText);
      ...
      if (isNewFor2001) notesParts.push('New for 2001');
      ...
      if (/costco/i.test(combinedText)) notesParts.push('Costco exclusive');
      if (/sam'?s club/i.test(combinedText)) notesParts.push("Sam's Club exclusive");
