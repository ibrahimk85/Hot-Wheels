/**
 * Import Silver Series - Vintage: Vintage Club (2026), National Icons (2025),
 * Salt Flat Racers (2025), Vintage Racing Club (2024)
 * Flexible table parser: 8-10 cols (Series#|Toy#|Casting|Color|...|Loose|Carded)
 * npx ts-node scripts/import/import_silver_series_vintage.ts
 */
...
const SERIES_CONFIG = [
  { url: 'https://hotwheels.fandom.com/wiki/Vintage_Club_Series_(2026)', year: 2026, seriesName: 'Vintage - Vintage Club (2026)' },
  { url: 'https://hotwheels.fandom.com/wiki/National_Icons_Series_(2025)', year: 2025, seriesName: 'Vintage - National Icons (2025)' },
  { url: 'https://hotwheels.fandom.com/wiki/Salt_Flat_Racers_Series_(2025)', year: 2025, seriesName: 'Vintage - Salt Flat Racers (2025)' },
  { url: 'https://hotwheels.fandom.com/wiki/Vintage_Racing_Club_Series_(2024)', year: 2024, seriesName: 'Vintage - Vintage Racing Club (2024)' },
];
...
function extractMixName($: cheerio.CheerioAPI, table: any): string {
  ...
  if (prev.length > 0) {
    ...
    if (!/^(contents|references|see also|external links|categories|gallery|vehicles)$/i.test(t)) return t;
  }
...
function parseRow($: cheerio.CheerioAPI, cells: cheerio.Cheerio<any>) {
  ...
  const notes = cells.length >= 10 ? (cells.length > 7 ? $(cells[7]).text().trim() : '') : (cells.length >= 9 ? (cells.length > 6 ? $(cells[6]).text().trim() : '') : (cells.length > 6 ? $(cells[6]).text().trim() : ''));
...
async function importSeries(config: typeof SERIES_CONFIG[0]) {
  ...
  for (let ti = 0; ti < tables.length; ti++) {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (!p.castingName || p.castingName.length < 2) continue;
      if (/^spectraflame|^chase$/i.test(p.castingName.trim())) continue;
      const toyNum = p.toyNumber || p.colNumber || '';
      if (!toyNum && !/^\d+\/\d+$/.test(p.colNumber)) continue;
      ...
      if (!model) {
        ...
        if (exM) model = { id: exM.id };
          ...
          if (href && !href.startsWith('#') && !href.includes('Spectraflame')) {
...
async function main() {
  ...
  console.log(`\nTotal: ${total} new variants across all Vintage series.`);
}
