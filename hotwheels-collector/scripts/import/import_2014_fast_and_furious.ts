/**
 * Script to import the 2014 Hot Wheels Fast & Furious Series set into your database using Prisma.
 *
 * This script:
 *   1. Fetches the Fast & Furious Series page from Hot Wheels Fandom wiki
 *   2. Parses the 2014 year table
 *   3. Extracts data: Series #, Casting Name, Color, Tampo, Wheel Type, Toy #, Base code(s)
 ...
 * - SubSeries: "2014" (single table, no sub-series)
 * - Model metadata from detail pages
 * - Photo Carded and Photo - Car (Loose) columns - will be handled by image download script
 ...
 *
 * How to use:
 *   npx ts-node scripts/import/import_2014_fast_and_furious.ts
 *
 * Notes:
 ...
 *   - Single table for year 2014 (no sub-series)
 */
...
async function main() {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    const baseCode = cells.length > 6 ? $(cells[6]).text().trim() : '';
    ...
    // Use baseCode as notes if available
    const finalNotes = baseCode || undefined;
