/**
 * Script to import the 2025 Hot Wheels Fast & Furious Premium set into your database using Prisma.
 *
 * This script:
 *   1. Fetches the 2025 Fast & Furious Premium section from Hot Wheels Fandom wiki
 ...
 * - Skips Tokyo Drift Bundle table
 *
 * How to use:
 *   npx ts-node scripts/import/import_2025_fast_furious_premium.ts
 *
 * Notes:
...
async function main() {
  ...
  // But skip Tokyo Drift Bundle table
  const allTables = $('table.wikitable');
  const tables = allTables.filter((_, table) => {
    const tableElement = $(table);
    
    // Extract sub-series name to check if it's Tokyo Drift Bundle
    const prevHeading = tableElement.prevAll('h2, h3, span.mw-headline').first();
    const headingText = prevHeading.text().trim();
    
    // Skip Tokyo Drift Bundle
    if (/tokyo drift bundle/i.test(headingText)) {
      return false;
    }
  ...
  for (let tableIdx = 0; tableIdx < tables.length; tableIdx++) {
    ...
    // Skip Tokyo Drift Bundle and other excluded sub-series
    if (/^(contents|references|see also|external links|categories|tokyo drift bundle)$/i.test(subSeriesName)) {
