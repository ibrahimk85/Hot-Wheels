/**
 * Import 2019 Hot Wheels Silver Series - American Pickup (Automotive category)
 * 8-col: Col # | Toy # | Casting | Color | Tampos | Wheel Type | Notes | Photo - Car | Photo - Card
 * npx ts-node scripts/import/import_2019_silver_series_american_pickup.ts
 */
...
const URL = 'https://hotwheels.fandom.com/wiki/American_Pickup_Series_(2019)';
...
async function main() {
  console.log(`Fetching American Pickup (${targetYear}) from ${URL}…`);
  const res = await fetch(URL, { headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' } });
