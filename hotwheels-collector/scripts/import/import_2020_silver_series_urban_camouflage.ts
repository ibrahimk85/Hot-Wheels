/**
 * Import 2020 Hot Wheels Silver Series - Urban Camouflage (Automotive category)
 * 8-col: Col # | Toy # | Casting | Body Color | Wheel Type | Notes | Photo Loose | Photo Carded
 * npx ts-node scripts/import/import_2020_silver_series_urban_camouflage.ts
 */
...
const URL = 'https://hotwheels.fandom.com/wiki/Urban_Camouflage_Series_(2020)';
...
async function main() {
  console.log(`Fetching Urban Camouflage (${targetYear}) from ${URL}…`);
  const res = await fetch(URL, { headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' } });
