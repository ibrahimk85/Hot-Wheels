/**
 * Import 2025 Hot Wheels Silver Series - Hybrid Speed (Automotive category)
 * Table: Series # | Toy # | Casting Name | Body Color | Tampo | Wheel Type | Notes | Photo loose | Photo carded (8 cols)
 * npx ts-node scripts/import/import_2025_silver_series_hybrid_speed.ts
 */
...
const URL = 'https://hotwheels.fandom.com/wiki/Hybrid_Speed_Series_(2025)';
const COLLECTION_NAME = 'Hot Wheels Silver Series';
const SERIES_NAME = 'Hybrid Speed (2025)';
...
/** 8-col: Series # | Toy # | Casting Name | Body Color | Tampo | Wheel Type | Notes | Photo loose | Photo carded */
...
async function main() {
  console.log(`Fetching Hybrid Speed (${targetYear}) from ${URL}…`);
  const res = await fetch(URL, { headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' } });
