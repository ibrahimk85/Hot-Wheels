/**
 * Script to import the 2019 Hot Wheels Mainline set into your database using Prisma.
 *
 * This script fetches the 2019 mainline table from the Hot Wheels Fandom wiki,
 * parses each row to extract the toy number, collector number, model name,
 * sub‑series, series info (e.g. New for 2019!, Treasure Hunt, Super Treasure Hunt,
 ...
 *      setup, the 2019 mainline data will be associated with the `Year`
 *      record for 2019 and the `Collection` record named "Mainline".
 *
 *   3. Run the script with ts-node:
 *
 *         npx ts-node import_2019_mainline.ts
 *
 *      Alternatively, if you have added a script entry in your package.json
 *      (e.g. "import:2019-mainline": "ts-node import_2019_mainline.ts"), you can
 *      run it via `npm run import:2019-mainline`.
 *
 * Notes:
...
// URL of the Fandom page containing the 2019 mainline list
...
async function main() {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    // Also remove "New for 2019!", "New in Mainline", etc. markers
    ...
    const isNewFor2019 = /New for 2019/i.test(seriesInfoRaw);
    ...
    if (isNewFor2019) notesParts.push('New for 2019');
