/**
 * Script to import the 2019 Pearl&Chrome Anniversary Series (Satin and Chrome Series)
 ...
 *
 * How to use:
 *   npx ts-node scripts/import/import_pearl_chrome_anniversary_2019.ts
 */
...
const SUB_SERIES_NAME = 'Satin and Chrome Series';
...
async function main() {
  ...
  // Find tables - check for Mix headings
  ...
  if (yearHeading.length > 0) {
    // Check if there are Mix headings
    ...
    if (mixHeadings.length > 0) {
      // Multiple tables (Mix 1, Mix 2, etc.)
      tablesToProcess = [];
