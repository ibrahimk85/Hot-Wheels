/**
 * Script to import the 2023 Hot Wheels Fast & Furious Premium set into your database using Prisma.
 *
 * This script:
 *   1. Fetches the 2023 Fast & Furious Premium section from Hot Wheels Fandom wiki
 ...
 * - Skips Fast & Furious Premium Bundle 1, 2, 3 tables
 *
 * How to use:
 *   npx ts-node scripts/import/import_2023_fast_furious_premium.ts
 *
 * Notes:
...
async function main() {
  ...
  // But skip Bundle 1, 2, 3 tables
  const allTables = $('table.wikitable');
  const tables = allTables.filter((_, table) => {
    const tableElement = $(table);
    
    // Extract sub-series name to check if it's a Bundle
    const prevHeading = tableElement.prevAll('h2, h3, span.mw-headline').first();
    const headingText = prevHeading.text().trim();
    
    // Skip Bundle 1, 2, 3
    if (/fast.*furious.*premium.*bundle.*1|fast.*furious.*premium.*bundle.*2|fast.*furious.*premium.*bundle.*3|bundle.*1|bundle.*2|bundle.*3/i.test(headingText)) {
      return false;
    }
  ...
  for (let tableIdx = 0; tableIdx < tables.length; tableIdx++) {
    ...
    // Skip Bundle 1, 2, 3 and other excluded sub-series
    if (/^(contents|references|see also|external links|categories|fast.*furious.*premium.*bundle.*1|fast.*furious.*premium.*bundle.*2|fast.*furious.*premium.*bundle.*3|bundle.*1|bundle.*2|bundle.*3)$/i.test(subSeriesName)) {
