/**
 * Script to import the 2008 Hot Wheels Mainline set into your database using Prisma.
 *
 * Fetches ALL mainline tables from the 2008 Fandom wiki page (multi-table layout).
 * Skips Walmart "Redline Exclusives" and "Goodyear Tire Exclusives" if present (same rules as 2009 script).
 *
 ...
 *
 *   npx ts-node scripts/import/import_2008_mainline.ts
 *
...
const URL = 'https://hotwheels.fandom.com/wiki/List_of_2008_Hot_Wheels';
...
async function main() {
  console.log('Fetching 2008 mainline data…');
  const html = await fetchFandomWikiHtml(URL);
  const $ = cheerio.load(html);

  const targetYear = 2008;
  ...
  for (const tableInfo of tablesToProcess) {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const isNewFor2008 = /New for 2008/i.test(combinedText);
      ...
      if (isNewFor2008) notesParts.push('New for 2008');
