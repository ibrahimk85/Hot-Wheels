/**
 * Import 2022 Hot Wheels Silver Series - Luxury Sedans (Automotive category)
 * npx ts-node scripts/import/import_2022_silver_series_luxury_sedans.ts
 */
...
const URL = 'https://hotwheels.fandom.com/wiki/Luxury_Sedans_Series_(2022)';
...
async function main() {
  console.log(`Fetching Luxury Sedans (${targetYear}) from ${URL}…`);
  const res = await fetch(URL, { headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' } });
