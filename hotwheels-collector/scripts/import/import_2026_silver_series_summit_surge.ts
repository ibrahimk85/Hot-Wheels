/**
 * Import 2026 Hot Wheels Silver Series - Summit Surge (Automotive category)
 * Table: Series # | Toy # | Casting Name | Color | Tampos | Base Color/Type | Wheel Type | Notes | Photo Loose | Photo Carded
 *
 * npx ts-node scripts/import/import_2026_silver_series_summit_surge.ts
 */
...
const URL = 'https://hotwheels.fandom.com/wiki/Summit_Surge_Series_(2026)';
...
async function main() {
  console.log(`Fetching Summit Surge (${targetYear}) from ${URL}…`);
  const res = await fetch(URL, { headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' } });
