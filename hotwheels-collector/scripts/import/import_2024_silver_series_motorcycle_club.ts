/**
 * Import 2024 Hot Wheels Silver Series - Motorcycle Club (Automotive category)
 * Table: Series # | Toy # | Casting Name | Color | Tampos | Base Color/Type | Wheel Type | Notes | Photo Loose | Photo Carded (10 cols)
 * npx ts-node scripts/import/import_2024_silver_series_motorcycle_club.ts
 */
...
const URL = 'https://hotwheels.fandom.com/wiki/Hot_Wheels_Motorcycle_Club_Series_(2024)';
const COLLECTION_NAME = 'Hot Wheels Silver Series';
const SERIES_NAME = 'Motorcycle Club (2024)';
...
/** 10-col: Series # | Toy # | Casting Name | Color | Tampos | Base Color/Type | Wheel Type | Notes | Photo Loose | Photo Carded */
...
async function main() {
  console.log(`Fetching Motorcycle Club (${targetYear}) from ${URL}…`);
  const res = await fetch(URL, { headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' } });
