/**
 * Import 2021 Hot Wheels Silver Series - Hot Pickups (Automotive category)
 * 8-col: Col # | Toy # | Casting Name | Color | Tampo | Wheel Type | Notes | Photo Loose | Photo Card
 * npx ts-node scripts/import/import_2021_silver_series_hot_pickups.ts
 */
...
const URL = 'https://hotwheels.fandom.com/wiki/Hot_Pickups_Series_(2021)';
const COLLECTION_NAME = 'Hot Wheels Silver Series';
const SERIES_NAME = 'Hot Pickups (2021)';
...
async function main() {
  console.log(`Fetching Hot Pickups (${targetYear}) from ${URL}…`);
  const res = await fetch(URL, { headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' } });
