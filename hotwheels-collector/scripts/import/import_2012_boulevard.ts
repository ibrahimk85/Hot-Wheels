/**
 * Script to import the 2012 Hot Wheels Boulevard set into your database using Prisma.
 *
 * This script:
 *   1. Fetches the 2012 Boulevard page from Hot Wheels Fandom wiki
 *   2. Parses multiple tables (themed Mixes: "Ahead of Its Time", "Big Hits", etc.)
 ...
 *   5. Creates database records: Year → Collection (Boulevard) → SubSeries (themed names) → Model → Variant
 *
 * Boulevard-specific:
 * - No TH/STH (always false)
 * - SubSeries are themed names: "Ahead of Its Time", "Big Hits", "Concept Cars", "Legends", "Show Rods", "Underdogs"
 * - Model metadata from detail pages
 * - No Series # column - use Toy # as cardNumber
 * - No Wheel Type column
 *
 * How to use:
 *   npx ts-node scripts/import/import_2012_boulevard.ts
 *
 * Notes:
 ...
 *   - "30-Car Set" and navigation tables are skipped
 */
...
const URL = 'https://hotwheels.fandom.com/wiki/2012_Hot_Wheels_Boulevard';
 ...
 * 2012 uses themed names: "Ahead of Its Time", "Big Hits", "Concept Cars", etc.
 */
function extractMixName($: cheerio.CheerioAPI, table: any): string {
  ...
  if (prevHeading.length > 0) {
    ...
    // Remove brackets and extra whitespace (e.g., "Ahead of Its Time[]" -> "Ahead of Its Time")
    mixName = headingText.replace(/\[\]/g, '').trim();
  }
  
  // Check table caption
  if (!mixName) {
    const caption = $(table).find('caption').text().trim();
    mixName = caption.replace(/\[\]/g, '').trim();
  }
  ...
  // Default to empty if not found (will skip)
  return mixName;
}
...
async function main() {
  ...
  for (let tableIdx = 0; tableIdx < tables.length; tableIdx++) {
    ...
    // Skip if no mix name found or if it's a special table
    if (!mixName || /30-car.*set/i.test(mixName) || /hot.*wheels.*boulevard/i.test(mixName)) {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      // Extract data based on known column structure for 2012
      // Column 0: Toy #
      ...
      await prisma.variant.create({
        data: {
          ...
          wheelType: undefined, // No Wheel Type column in 2012
