/**
 * Import 2026 Hot Wheels Silver Series - Spider-Man (Entertainment category)
 * 10-col: Series # | Toy # | Casting | Color | Tampos | Base Color/Type | Wheel Type | Notes | Photo Loose | Photo Carded
 * npx ts-node scripts/import/import_2026_silver_series_spider_man.ts
 */
...
const URL = 'https://hotwheels.fandom.com/wiki/Spider-Man_Series_(2026)';
...
async function main() {
  console.log(`Fetching Spider-Man (${targetYear}) from ${URL}…`);
  const res = await fetch(URL, { headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' } });
