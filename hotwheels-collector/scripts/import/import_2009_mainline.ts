/**
 * Script to import the 2009 Hot Wheels Mainline set into your database using Prisma.
 *
 * Fetches ALL mainline tables from the 2009 Fandom wiki page (multi-table layout).
 * Skips Walmart "Redline Exclusives" (duplicate rows) and "Goodyear Tire Exclusives" (2010 release).
 *
 ...
 *
 *   npx ts-node scripts/import/import_2009_mainline.ts
 *
...
const URL = 'https://hotwheels.fandom.com/wiki/List_of_2009_Hot_Wheels';
...
async function main() {
  console.log('Fetching 2009 mainline data…');
  ...
  for (const tableInfo of tablesToProcess) {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const isNewFor2009 = /New for 2009/i.test(combinedText);
      ...
      if (isNewFor2009) notesParts.push('New for 2009');
