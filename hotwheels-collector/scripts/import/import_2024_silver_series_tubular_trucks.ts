/**
 * Import 2024 Hot Wheels Silver Series - Tubular Trucks (Automotive category)
 * Table: Series # | Toy # | Casting Name | Color | Tampos | Base Color/Type | Wheel Type | Notes | Photo Loose | Photo Carded (10 cols)
 * npx ts-node scripts/import/import_2024_silver_series_tubular_trucks.ts
 */
...
const URL = 'https://hotwheels.fandom.com/wiki/Tubular_Trucks_Series_(2024)';
const COLLECTION_NAME = 'Hot Wheels Silver Series';
const SERIES_NAME = 'Tubular Trucks (2024)';
...
async function main() {
  console.log(`Fetching Tubular Trucks (${targetYear}) from ${URL}…`);
  const res = await fetch(URL, { headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' } });
