/**
 * Import 2025 Hot Wheels Silver Series - '90s Street Scene (Automotive category)
 * Table: Series # | Toy # | Casting Name | Color | Tampos | Base Color/Type | Wheel Type | Notes | Photo Loose | Photo Carded
 * npx ts-node scripts/import/import_2025_silver_series_90s_street_scene.ts
 */
...
const URL = "https://hotwheels.fandom.com/wiki/%2790s_Street_Scene_Series_(2025)";
const COLLECTION_NAME = 'Hot Wheels Silver Series';
const SERIES_NAME = "'90s Street Scene (2025)";
...
async function main() {
  console.log(`Fetching '90s Street Scene (${targetYear}) from ${URL}…`);
  const res = await fetch(URL, { headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' } });
