/**
 * Script to import Elite 64 Future collection data and images
 * 
 * This script:
 * 1. Fetches the Elite 64 wiki page
 * 2. Finds the Future year section
 * 3. Reads all sub-series for Future
 ...
 * 
 * Usage:
 *   npx ts-node scripts/import/import_elite64_future.ts
 */
...
const TARGET_YEAR = 9999; // Special year for Future
const YEAR_SEARCH_TEXT = 'Future';
...
async function processYearSection(
  ...
  if (!table || table.length === 0) {
    console.warn(`No table found for Future section`);
    return;
  }
  
  console.log(`Found table for Future with ${table.find('tbody tr').length} rows`);

  // Get or create Year record for Future (using 9999)
  ...
  if (!yearRecord) {
    ...
    console.log(`Created Year record for Future (${TARGET_YEAR})`);
  }

  // Get or create Collection record for Elite 64 Future
  ...
  if (!collectionRecord) {
    collectionRecord = await prisma.collection.create({
      data: {
        name: 'Elite 64',
        code: 'Elite 64',
        isFuture: true, // Mark as Future collection
        year: { connect: { id: yearRecord.id } },
      },
    });
    console.log(`Created Collection record for Elite 64 Future`);
  }

  const yearFolder = 'future';
  const baseDir = path.join(process.cwd(), 'public', 'images', 'hotwheels', yearFolder, 'elite64');
  await fs.promises.mkdir(baseDir, { recursive: true });

  const tbodyRows = table.find('tbody tr');
  console.log(`\nProcessing ${tbodyRows.length} rows for Future...`);
...
async function main() {
  ...
  allHeaders.each((_, el) => {
    ...
    if (text.toLowerCase().includes(YEAR_SEARCH_TEXT.toLowerCase())) {
      yearHeader = el;
      return false;
    }
  });
  ...
  if (!yearHeader) {
    console.log(`Future section not found, skipping...`);
    return;
  }
  
  console.log(`Found header for Future: "${$(yearHeader).text().trim()}"`);
  await processYearSection($, yearHeader);

  console.log('\n=== FINAL SUMMARY ===');
  console.log('Elite 64 Future import completed successfully!');
}
