/**
 * Import 2021 Hot Wheels Silver Series - Forza Horizon (Automotive category)
 * 8-col: Col # | Toy # | Casting Name | Body Color | Wheel Type | Notes | Photo 1 Loose | Photo 2 Card
 * npx ts-node scripts/import/import_2021_silver_series_forza_horizon.ts
 */
...
const URL = 'https://hotwheels.fandom.com/wiki/Forza_Horizon_Series_(2021)';
...
async function main() {
  console.log(`Fetching Forza Horizon (${targetYear}) from ${URL}…`);
  const res = await fetch(URL, { headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' } });
