/**
 * Script to import the 2006 Hot Wheels Mainline set into your database using Prisma.
 *
 * Fetches ALL mainline tables from the 2006 Fandom wiki page (multi-table layout):
 * 2006 First Editions, 2006 Treasure Hunts Series, Segment Series (12 subs), Track Aces,
 * 2006 Open Stock, 2006 Mystery Car Series.
 * Skips Walmart "Redline Exclusives" and "Goodyear Tire Exclusives" if present (same rules as 2007+).
 * Note: Segment "Red Line" is a normal series name, not Walmart Redline Exclusives (skip uses "Redline Exclusives").
 *
 ...
 *
 *   npx ts-node scripts/import/import_2006_mainline.ts
 *
...
const URL = 'https://hotwheels.fandom.com/wiki/List_of_2006_Hot_Wheels';
...
async function main() {
  console.log('Fetching 2006 mainline data…');
  ...
  for (const tableInfo of tablesToProcess) {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const isNewFor2006 = /New for 2006/i.test(combinedText);
      ...
      if (isNewFor2006) notesParts.push('New for 2006');
