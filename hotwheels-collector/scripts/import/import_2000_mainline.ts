/**
 * Script to import the 2000 Hot Wheels Mainline set into your database using Prisma.
 *
 * Fetches ALL mainline tables from the 2000 Fandom wiki page (multi-table layout):
 * Mainline Segment (Future Fleet 2000, Hot Rod Magazine, Seein' 3-D, Snack Time, Mad Maniax, Attack Pack,
 * Circus on Wheels, CD Customs, Kung Fu Force, Speed Blaster, Tony Hawk Skate, Secret Code),
 * 2000 Treasure Hunts, 2000 First Editions, Virtual Collection, 2000 Hot Wheels, 2000 Bonus Cars.
 * Skips Walmart "Redline Exclusives" and "Goodyear Tire Exclusives" if present (same rules as 2001+).
 *
 * IMPORTANT: Same Toy# can repeat for 2nd/3rd/4th Color rows (same COL#). Variants dedupe by
 * (modelId, toyNumber, year, color). FTE rows share Toy# with base but differ by SubSeries/model.
 * combinedText includes heading context for TH/STH flags (Treasure Hunts, T-Hunt spellings).
 * 2000 Bonus Cars rows often omit Toy# — synthetic `2000-BC-{Col#}-{casting}` keys (same idea as 2001 Store Exclusives).
 *
 *   npx ts-node scripts/import/import_2000_mainline.ts
 *
...
const URL = 'https://hotwheels.fandom.com/wiki/List_of_2000_Hot_Wheels';
...
/** MediaWiki API HTML sometimes leaves `[]` or zero-width chars in headings/cells */
...
async function main() {
  console.log('Fetching 2000 mainline data…');
  ...
  allTables.each((index, tableElement) => {
    ...
    // Treasure Hunts table: series column sometimes parses as "s" or "Series"
    if (/^s$/i.test(subSeriesName) || /^series$/i.test(subSeriesName)) {
      subSeriesName = heading ? stripFandomApiArtefacts(heading) : subSeriesName;
    }
    // e.g. wiki cell "2000s" while h2 is "2000 Treasure Hunts"
    if (/^\d{4}s$/i.test(subSeriesName) && heading && /treasure/i.test(heading)) {
      subSeriesName = stripFandomApiArtefacts(heading);
    }
    if (/\bbonus car/i.test(heading ?? '') || /\b2000\s+bonus car/i.test(subSeriesName)) {
      subSeriesName = 'Bonus Cars';
    }
  ...
  for (const tableInfo of tablesToProcess) {
    const { subSeriesName, table, headingContext, heading } = tableInfo;
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const collectorNumberStr = stripFandomApiArtefacts($(cells[1]).text().trim());
      ...
      const seriesInfoRaw = stripFandomApiArtefacts(
        cells.length >= 6 ? $(cells[4]).text().trim() : $(cells[4] || cells[3]).text().trim(),
      );

      let castingName = modelNameRaw;
      let variantDescription: string | null = null;
      const variantMatchEarly = modelNameRaw.match(/^(.*?)\s*\(([^)]+)\)$/);
      if (variantMatchEarly) {
        castingName = variantMatchEarly[1].trim();
        const parsedDescription = variantMatchEarly[2].trim();
        if (parsedDescription.toLowerCase() !== 'mainline') {
          variantDescription = parsedDescription;
        }
      }

      let toyNumber = stripFandomApiArtefacts($(cells[0]).text().trim());
      if (!toyNumber) {
        const bonusCtx = `${heading ?? ''} ${headingContext} ${subSeriesName} ${subSeriesNameRaw}`;
        if (/bonus car/i.test(bonusCtx)) {
          toyNumber = `${targetYear}-BC-${slugify(collectorNumberStr || `row${i}`)}-${slugify(castingName)}`.replace(
            /-+/g,
            '-',
          );
        } else {
      ...
      let cleanedSubSeriesName = stripThFromSeriesName(subSeriesNameRaw)
        ...
        .replace(/\s*New for 2000!\s*/gi, '')
        .replace(/\s*New in Mainline\s*/gi, '')
        .replace(/\s*New for 2000\s*/gi, '')
        .trim();

      cleanedSubSeriesName = stripFandomApiArtefacts(cleanedSubSeriesName);
      // TH table quirk: series cell often parses as "s" — use table subSeriesName
      if (/^s$/i.test(cleanedSubSeriesName) || /^series$/i.test(cleanedSubSeriesName)) {
        cleanedSubSeriesName = '';
      }
      if (/^\d{4}s$/i.test(cleanedSubSeriesName)) {
        cleanedSubSeriesName = '';
      }
      ...
      if (subSeriesName === 'Bonus Cars') {
        finalSubSeriesName = 'Bonus Cars';
      }

      const combinedText = `${subSeriesNameRaw} ${seriesInfoRaw} ${headingContext}`;
      const isSuperTreasureHunt =
        /Super\s+Treas(?:ure|ue)\s+Hunt/i.test(combinedText) ||
        /Super\s+T-?\s*Hunt/i.test(combinedText);
      const isTreasureHunt =
        !isSuperTreasureHunt &&
        (/Treas(?:ure|ue)\s+Hunt/i.test(combinedText) || /\bT-?\s*Hunt\b/i.test(combinedText));
      ...
      const isNewFor2000 = /New for 2000/i.test(combinedText);
      ...
      if (isNewFor2000) notesParts.push('New for 2000');
      ...
      if (/bonus car/i.test(combinedText)) notesParts.push('Bonus Car');
