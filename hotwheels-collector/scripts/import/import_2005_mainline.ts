/**
 * Script to import the 2005 Hot Wheels Mainline set into your database using Prisma.
 *
 * Fetches ALL mainline tables from the 2005 Fandom wiki page (multi-table layout):
 * 2005 First Editions (Realistix, Drop Tops, Blings, Torpedoes, X-Raycers), Segment Series
 * (Track Aces, Final Run, Rebel Rides, Asphalt Jungle, HW Racing, Pin Hedz, Red Lines Series,
 * Muscle Mania, White Heat, Crazed Clowns II, Twenty+), 2005 Treasure Hunts, 2005 Hot Wheels,
 * Mystery Car Series.
 * Skips Walmart "Redline Exclusives" and "Goodyear Tire Exclusives" if present (same rules as 2006+).
 * Note: Segment "Red Lines Series" is a normal series name, not Walmart Redline Exclusives.
 *
 ...
 *
 *   npx ts-node scripts/import/import_2005_mainline.ts
 *
...
const URL = 'https://hotwheels.fandom.com/wiki/List_of_2005_Hot_Wheels';
...
async function main() {
  console.log('Fetching 2005 mainline data…');
  ...
  for (const tableInfo of tablesToProcess) {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const isNewFor2005 = /New for 2005/i.test(combinedText);
      ...
      if (isNewFor2005) notesParts.push('New for 2005');
