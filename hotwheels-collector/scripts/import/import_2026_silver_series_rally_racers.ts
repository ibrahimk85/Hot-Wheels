/**
 * Import 2026 Hot Wheels Silver Series - Rally Racers (Automotive category)
 * Table: Series # | Toy # | Casting Name | Color | Tampos | Base Color/Type | Wheel Type | Notes | Photo Loose | Photo Carded
 *
 * npx ts-node scripts/import/import_2026_silver_series_rally_racers.ts
 */
...
const URL = 'https://hotwheels.fandom.com/wiki/Rally_Racers_Series_(2026)';
const COLLECTION_NAME = 'Hot Wheels Silver Series';
const SERIES_NAME = 'Rally Racers (2026)';
...
async function main() {
  console.log(`Fetching Rally Racers (${targetYear}) from ${URL}…`);
  const res = await fetch(URL, { headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' } });
