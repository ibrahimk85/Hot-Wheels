/**
 * Script to import the 2018 Hot Wheels Mainline set into your database using Prisma.
 *
 * This script fetches the 2018 mainline table from the Hot Wheels Fandom wiki,
 * parses each row to extract the toy number, collector number, model name,
 * sub‑series, series info (e.g. New for 2018!, Treasure Hunt, Super Treasure Hunt,
 ...
 *      setup, the 2018 mainline data will be associated with the `Year`
 *      record for 2018 and the `Collection` record named "Mainline".
 *
 *   3. Run the script with ts-node:
 *
 *         npx ts-node import_2018_mainline.ts
 *
 *      Alternatively, if you have added a script entry in your package.json
 *      (e.g. "import:2018-mainline": "ts-node import_2018_mainline.ts"), you can
 *      run it via `npm run import:2018-mainline`.
 *
 * Notes:
...
// URL of the Fandom page containing the 2018 mainline list
const URL = 'https://hotwheels.fandom.com/wiki/List_of_2018_Hot_Wheels';
