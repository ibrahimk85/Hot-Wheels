/**
 * Import 2019 Hot Wheels Silver Series - Exotics (Automotive category)
 * 8-col: Series # | Toy # | Casting | Color | Tampos | Wheel Type | Notes | Photo - Car | Photo - Card
 * npx ts-node scripts/import/import_2019_silver_series_exotics.ts
 */
...
const URL = 'https://hotwheels.fandom.com/wiki/Exotics_Series_(2019)';
...
async function main() {
  console.log(`Fetching Exotics (${targetYear}) from ${URL}…`);
  const res = await fetch(URL, { headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' } });
