/**
 * Import 2024 Hot Wheels Silver Series - J-Imports (Automotive category)
 * Table: Series # | Toy # | Casting Name | Color | Tampo | Wheel Type | Notes | Photo Loose | Photo Carded (8 cols)
 * npx ts-node scripts/import/import_2024_silver_series_j_imports.ts
 */
...
const URL = 'https://hotwheels.fandom.com/wiki/Hot_Wheels_J-Imports_Series_(2024)';
...
/** 8-col: Series # | Toy # | Casting Name | Color | Tampo | Wheel Type | Notes | Photo Loose | Photo Carded */
...
async function main() {
  console.log(`Fetching J-Imports (${targetYear}) from ${URL}…`);
  const res = await fetch(URL, { headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' } });
