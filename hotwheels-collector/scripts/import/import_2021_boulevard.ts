/**
 * Script to import the 2021 Hot Wheels Boulevard set into your database using Prisma.
 *
 * This script:
 *   1. Fetches the 2021 Boulevard page from Hot Wheels Fandom wiki
 ...
 *
 * How to use:
 *   npx ts-node scripts/import/import_2021_boulevard.ts
 *
 * Notes:
 *   - The script is idempotent for Year, Collection, and SubSeries
 *   - Running multiple times may create duplicate variants - clean DB if needed
...
const URL = 'https://hotwheels.fandom.com/wiki/2021_Hot_Wheels_Boulevard';
...
function extractMixName($: cheerio.CheerioAPI, table: any): string {
  ...
  // Check previous h2, h3, h4 elements (look for "Mix 1", "Mix 2", etc.)
  ...
  // Check if table has rows with data - if not, return empty to skip
  const rows = $(table).find('tbody tr').filter((_: any, row: any) => {
    const cells = $(row).find('td');
    return cells.length >= 3; // At least Toy #, Series #, Casting Name
  });
  
  // If no rows found, return empty string to skip this table
  if (rows.length === 0) {
    return '';
  }
  
  // Default to Mix 1 if not found but has rows
...
async function main() {
  ...
  for (let tableIdx = 0; tableIdx < tables.length; tableIdx++) {
    ...
    // Skip if no mix name found (empty table or couldn't determine Mix)
    if (!mixName || mixName === '') {
      console.log(`Skipping table ${tableIdx + 1} (could not determine Mix name or empty)`);
      continue;
    }
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      // Casting Name is in a link in column 2, but may also be plain text
      ...
      if (castingNameLink.length > 0) {
        ...
        // If no link, try to get text from the cell
        castingNameRaw = $(cells[2]).text().trim();
        // Sometimes the text might have extra whitespace or newlines
        castingNameRaw = castingNameRaw.replace(/\s+/g, ' ').trim();
      }
      ...
      if (!toyNumber || !seriesNumber || !castingNameRaw) {
        console.warn(`Skipping row ${i + 1} in ${mixName} with missing data: Toy#="${toyNumber}", Series#="${seriesNumber}", Name="${castingNameRaw}"`);
        console.warn(`  Cell count: ${cells.length}, Cell 2 content: "${$(cells[2]).html()?.substring(0, 100)}"`);
        continue;
      }
      ...
      if (existingVariant) {
        // Variant already exists, but we still processed the row
        totalProcessed++;
        continue;
      }
