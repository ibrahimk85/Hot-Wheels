/**
 * Import 2026 Hot Wheels Silver Series - Compact Kings (Automotive category)
 * Table: Series # | Toy # | Casting Name | Color | Tampos | Base Color/Type | Wheel Type | Notes | Photo Loose | Photo Carded
 *
 * npx ts-node scripts/import/import_2026_silver_series_compact_kings.ts
 */
...
const URL = 'https://hotwheels.fandom.com/wiki/Compact_Kings_Series_(2026)';
const COLLECTION_NAME = 'Hot Wheels Silver Series';
const SERIES_NAME = 'Compact Kings (2026)';
...
/** Automotive table: Series # | Toy # | Casting Name | Color | Tampos | Base Color/Type | Wheel Type | Notes | Photo Loose | Photo Carded */
...
async function main() {
  console.log(`Fetching Compact Kings (${targetYear}) from ${URL}…`);
  const res = await fetch(URL, { headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' } });
