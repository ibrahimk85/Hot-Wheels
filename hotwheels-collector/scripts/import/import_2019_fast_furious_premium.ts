/**
 * Script to import the 2019 Hot Wheels Fast & Furious Premium set into your database using Prisma.
 *
 * This script:
 *   1. Fetches the 2019 Fast & Furious Premium section from Hot Wheels Fandom wiki
 ...
 * - Skips Boxed Set tables
 *
 * How to use:
 *   npx ts-node scripts/import/import_2019_fast_furious_premium.ts
 *
 * Notes:
...
async function main() {
  ...
  // But skip Boxed Set tables
  const allTables = $('table.wikitable');
  const tables = allTables.filter((_, table) => {
    const tableElement = $(table);
    
    // Extract sub-series name to check if it's Boxed Set
  ...
  for (let tableIdx = 0; tableIdx < tables.length; tableIdx++) {
    ...
    // Skip Boxed Set and other excluded sub-series
