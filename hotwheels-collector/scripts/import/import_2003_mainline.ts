/**
 * Script to import the 2003 Hot Wheels Mainline set into your database using Prisma.
 *
 * Fetches ALL mainline tables from the 2003 Fandom wiki page (multi-table layout):
 * Treasure Hunts, 2003 First Editions, Segment Series (Wild Wave, Flamin' Hot Wheels, Dragon Wagons,
 * HW Anime, Flying Aces II, Boulevard Buccaneers, Carbonated Cruisers, Radical Wrestlers,
 * Crazed Clowns, Tech Tuners, Spectraflame II, Sega Games, Alt Terrain, Pride Rides, Roll Patrol,
 * Track Aces, Wastelanders, Work Crewsers), 2003 Hot Wheels, Final Run.
 * Skips Walmart "Redline Exclusives" and "Goodyear Tire Exclusives" if present (same rules as 2004+).
 *
 ...
 * combinedText includes heading context for TH/STH flags (Treasure Hunts section, T-Hunt spellings).
 *
 *   npx ts-node scripts/import/import_2003_mainline.ts
 *
...
const URL = 'https://hotwheels.fandom.com/wiki/List_of_2003_Hot_Wheels';
...
async function main() {
  console.log('Fetching 2003 mainline data…');
  ...
  allTables.each((index, tableElement) => {
    ...
    // Treasure Hunts table on 2003 wiki: series column sometimes parses as "s" or "Series"
  ...
  for (const tableInfo of tablesToProcess) {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      let cleanedSubSeriesName = stripThFromSeriesName(subSeriesNameRaw)
        ...
        .replace(/\s*New for 2003!\s*/gi, '')
        .replace(/\s*New in Mainline\s*/gi, '')
        .replace(/\s*New for 2003\s*/gi, '')
        .trim();

      cleanedSubSeriesName = stripFandomApiArtefacts(cleanedSubSeriesName);
      // Same 2003 TH table quirk: series cell often parses as "s" — use table subSeriesName
      ...
      const isNewFor2003 = /New for 2003/i.test(combinedText);
      ...
      if (isNewFor2003) notesParts.push('New for 2003');
