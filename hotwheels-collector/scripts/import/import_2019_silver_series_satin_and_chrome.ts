/**
 * Script to import the 2019 Hot Wheels Silver Series - Satin and Chrome set into your database.
 *
 * Fetches from: https://hotwheels.fandom.com/wiki/Satin_and_Chrome_Series_(2019)
 * Creates: Year → Collection (Hot Wheels Silver Series) → SubSeries (category=Anniversary) → Model → Variant
 *
 * How to use:
 *   npx ts-node scripts/import/import_2019_silver_series_satin_and_chrome.ts
 */
...
const URL = 'https://hotwheels.fandom.com/wiki/Satin_and_Chrome_Series_(2019)';
const COLLECTION_NAME = 'Hot Wheels Silver Series';
const SERIES_NAME = 'Satin and Chrome (2019)';
...
async function main() {
  console.log(`Fetching Satin and Chrome Series (${targetYear}) from ${URL}…`);
