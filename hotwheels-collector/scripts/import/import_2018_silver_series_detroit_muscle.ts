/**
 * Import 2018 Hot Wheels Silver Series - Detroit Muscle (Automotive category)
 * 8-col: Col # | Toy # | Casting | Color | Tampos | Wheel Type | Notes | Photo Loose | Photo Carded
 * npx ts-node scripts/import/import_2018_silver_series_detroit_muscle.ts
 */
...
const URL = 'https://hotwheels.fandom.com/wiki/Detroit_Muscle_Series_(2018)';
...
async function main() {
  console.log(`Fetching Detroit Muscle (${targetYear}) from ${URL}…`);
  const res = await fetch(URL, { headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' } });
