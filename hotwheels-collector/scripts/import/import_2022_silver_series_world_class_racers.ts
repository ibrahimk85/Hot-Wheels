/**
 * Import 2022 Hot Wheels Silver Series - World Class Racers (Automotive category)
 * npx ts-node scripts/import/import_2022_silver_series_world_class_racers.ts
 */
...
const URL = 'https://hotwheels.fandom.com/wiki/World_Class_Racers_Series_(2022)';
...
async function main() {
  console.log(`Fetching World Class Racers (${targetYear}) from ${URL}…`);
  const res = await fetch(URL, { headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' } });
