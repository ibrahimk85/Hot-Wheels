/**
 * Import 2019 Hot Wheels Silver Series - Forza Horizon 4 (Automotive category)
 * 8-col: Series # | Toy # | Casting | Color | Tampos | Wheel Type | Notes | Photo - Car | Photo - Card
 * npx ts-node scripts/import/import_2019_silver_series_forza_horizon_4.ts
 */
...
const URL = 'https://hotwheels.fandom.com/wiki/Forza_Horizon_4_Series_(2019)';
...
async function main() {
  console.log(`Fetching Forza Horizon 4 (${targetYear}) from ${URL}…`);
  const res = await fetch(URL, { headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' } });
