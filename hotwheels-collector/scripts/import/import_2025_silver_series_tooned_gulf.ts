/**
 * Import 2025 Hot Wheels Silver Series - Tooned Gulf (Automotive category)
 * Table: Series # | Toy # | Casting Name | Color | Tampos | Base Color/Type | Wheel Type | Notes | Photo Loose | Photo Carded
 * npx ts-node scripts/import/import_2025_silver_series_tooned_gulf.ts
 */
...
const URL = 'https://hotwheels.fandom.com/wiki/Tooned_Gulf_Series_(2025)';
const COLLECTION_NAME = 'Hot Wheels Silver Series';
const SERIES_NAME = 'Tooned Gulf (2025)';
...
function parseRow($: cheerio.CheerioAPI, cells: cheerio.Cheerio<any>) {
  ...
  const wheelType = cells.length > 6 ? $(cells[6]).text().trim() : cells.length > 5 ? $(cells[5]).text().trim() : '';
  const notes = cells.length > 7 ? $(cells[7]).text().trim() : cells.length > 6 ? $(cells[6]).text().trim() : '';
  return { colNumber, toyNumber, castingName, color, wheelType, notes, castingNameLink: link };
}

async function main() {
  console.log(`Fetching Tooned Gulf (${targetYear}) from ${URL}…`);
  const res = await fetch(URL, { headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' } });
