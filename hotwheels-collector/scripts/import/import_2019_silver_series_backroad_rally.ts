/**
 * Import 2019 Hot Wheels Silver Series - Backroad Rally (Automotive category)
 * 12-col: Casting Name | No. | Toy # | Color | Tampo | Base | Window | Interior | Wheel Type | Notes | Photo | Photo Card
 * npx ts-node scripts/import/import_2019_silver_series_backroad_rally.ts
 */
...
const URL = 'https://hotwheels.fandom.com/wiki/Backroad_Rally_Series_(2019)';
const COLLECTION_NAME = 'Hot Wheels Silver Series';
const SERIES_NAME = 'Backroad Rally (2019)';
...
/** 12-col: Casting | No. | Toy # | Color | Tampo | Base | Window | Interior | Wheel Type | Notes | Photo | Photo Card */
function parseRow($: cheerio.CheerioAPI, cells: cheerio.Cheerio<any>) {
  const link = $(cells[0]).find('a').first();
  const castingName = link.length > 0 ? link.text().trim() : cells.length > 0 ? $(cells[0]).text().trim() : '';
  const colNumber = cells.length > 1 ? $(cells[1]).text().trim() : '';
  const toyNumber = cells.length > 2 ? $(cells[2]).text().trim() : '';
  const color = cells.length > 3 ? $(cells[3]).text().trim() : '';
  const wheelType = cells.length > 8 ? $(cells[8]).text().trim() : '';
  const notes = cells.length > 9 ? $(cells[9]).text().trim() : '';
  return { colNumber, toyNumber, castingName, color, wheelType, notes, castingNameLink: link };
}

async function main() {
  console.log(`Fetching Backroad Rally (${targetYear}) from ${URL}…`);
  const res = await fetch(URL, { headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' } });
