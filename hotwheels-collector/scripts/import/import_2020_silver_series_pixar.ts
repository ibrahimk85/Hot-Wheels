...
const URL = 'https://hotwheels.fandom.com/wiki/Pixar_Series_(2020)';
...
function parseRow($: cheerio.CheerioAPI, cells: cheerio.Cheerio<any>) {
  const isPixar = cells.length >= 9 && cells.length <= 10; // Col# | Casting | Color | Theme | Wheel | Toy# | Notes | Loose | Carded
  const colNumber = cells.length > 0 ? $(cells[0]).text().trim() : '';
  const toyNumber = isPixar && cells.length > 5 ? $(cells[5]).text().trim() : (cells.length > 1 ? $(cells[1]).text().trim() : '');
  ...
  const wheelType = isPixar ? (cells.length > 4 ? $(cells[4]).text().trim() : '') : (cells.length > 5 ? $(cells[5]).text().trim() : '');
  const notes = isPixar ? (cells.length > 6 ? $(cells[6]).text().trim() : '') : (cells.length > 6 ? $(cells[6]).text().trim() : '');
  return { colNumber, toyNumber, castingName, color, wheelType, notes, castingNameLink: link };
}

async function main() {
  console.log(`Fetching Pixar (${targetYear}) from ${URL}…`);
  const res = await fetch(URL, { headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' } });
