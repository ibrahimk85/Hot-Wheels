/**
 * Script to import the 2017 Hot Wheels Mainline set into your database using Prisma.
 *
 * This script fetches the 2017 mainline table from the Hot Wheels Fandom wiki,
 * parses each row to extract the toy number, collector number, model name,
 * sub‑series, series info (e.g. New for 2017!, Treasure Hunt, Super Treasure Hunt,
 ...
 *      setup, the 2017 mainline data will be associated with the `Year`
 *      record for 2017 and the `Collection` record named "Mainline".
 *
 *   3. Run the script with ts-node:
 *
 *         npx ts-node import_2017_mainline.ts
 *
 *      Alternatively, if you have added a script entry in your package.json
 *      (e.g. "import:2017-mainline": "ts-node import_2017_mainline.ts"), you can
 *      run it via `npm run import:2017-mainline`.
 *
 * Notes:
...
// URL of the Fandom page containing the 2017 mainline list
...
async function main() {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    // Clean subSeriesName - remove TH/STH markers for the series name itself
    ...
    // Check both subSeriesName (Series column) and seriesInfoRaw (Series# column) for TH/STH
    ...
    const isNewFor2017 = /New for 2017/i.test(seriesInfoRaw);
    ...
    if (isNewFor2017) notesParts.push('New for 2017');
    ...
    await prisma.variant.create({
      data: {
        ...
        color: colorVariant ?? undefined,
        cardNumber: collectorNumberStr,
        // @ts-ignore - toyNumber field exists in database after migration, Prisma client will be regenerated
        toyNumber: toyNumber, // IMPORTANT: Save Toy# for each variant (1st Color, 2nd Color, 3rd Color have different Toy#)
        isTreasureHunt,
        isSuperTreasureHunt,
        wheelType: undefined,
