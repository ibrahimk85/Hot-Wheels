/**
 * Script to import the 2026 Hot Wheels Team Transport set into your database using Prisma.
 *
 * This script:
 *   1. Fetches the 2026 Team Transport page from Hot Wheels Fandom wiki
 ...
 *
 * How to use:
 *   npx ts-node scripts/import/import_2026_team_transport.ts
 */
...
const URL = 'https://hotwheels.fandom.com/wiki/2026_Car_Culture:_Team_Transport';
...
async function main() {
  ...
  for (let tableIdx = 0; tableIdx < tables.length; tableIdx++) {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      // Debug for Supreme Exclusive / Walmart Exclusive / Fast & Furious
      if (isSupremeExclusive && i < 2) {
        console.log(`  ${subSeriesName} Row ${i} cells (${cellValues.length}): ${cellValues.map((v, idx) => `[${idx}]${v.substring(0, 30)}`).join(' | ')}`);
      }
      ...
      // For Supreme Exclusive/Walmart/Fast & Furious, Series# can be "N/A"
