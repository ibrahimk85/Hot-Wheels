/**
 * Script to import the 2026 Hot Wheels Mainline set into your database using Prisma.
 *
 * This script fetches the 2026 mainline table from the Hot Wheels Fandom wiki,
 * parses each row to extract the toy number, collector number, model name,
 * sub‑series, series info (e.g. New for 2026!, Treasure Hunt, Super Treasure Hunt,
 ...
 *
 * Important: This script uses Toy# for variant identification and matching.
 *
 * How to use:
 *
 ...
 *      setup, the 2026 mainline data will be associated with the `Year`
 *      record for 2026 and the `Collection` record named "Mainline".
 *
 *   3. Run the script with ts-node:
 *
 *         npx ts-node scripts/import/import_2026_mainline.ts
 *
 * Notes:
 ...
 *     number but have unique toy numbers. This script uses Toy# for
 *     variant identification and matching.
 */
...
/**
 * Import 2026 Mainline from the wiki list page. Uses `scripts/config/mainline_urls.json`
 * when `wikiUrl` is omitted.
 */
export async function runImport2026Mainline(
  prisma: PrismaClient,
  wikiUrl?: string,
): Promise<void> {
  const url = wikiUrl ?? getMainlineWikiUrlForYear(TARGET_YEAR);
  console.log('Fetching 2026 mainline data…');
  const html = await fetchFandomWikiHtml(url);
  const $ = cheerio.load(html);

  // Locate the table containing the list. The table is the first
  // table after the header "2026 Hot Wheels #1–250".
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    // Extract TH/STH from subSeriesName (they are concatenated without space, e.g. "NightspeedTreasure Hunt")
    // Also check seriesInfo for completeness
    ...
    // Clean subSeriesName by removing TH/STH text and exclusive info
    // Note: "Super" might remain after "Super Treasure Hunt" removal, so remove it separately
    let subSeriesName = subSeriesNameRaw
      .replace(/Super Treasure Hunt/gi, '')
      .replace(/Treasure Hunt/gi, '')
      .replace(/Target Exclusive/gi, '')
      .replace(/Walmart Exclusive/gi, '')
      .replace(/Kroger Exclusive/gi, '')
      .replace(/^Super\s+/gi, '') // Remove "Super" at the start
      .replace(/\s+Super$/gi, '') // Remove "Super" at the end
      .replace(/\s+Super\s+/gi, ' ') // Remove "Super" in the middle
      .trim();
    
    // Extract exclusive flags from both subSeriesName and seriesInfo
    const isRedEdition = /Red Edition/i.test(seriesInfoRaw) || /Red Edition/i.test(subSeriesNameRaw);
    const isTargetExclusive = /Target Exclusive/i.test(seriesInfoRaw) || /Target Exclusive/i.test(subSeriesNameRaw);
    const isWalmartExclusive = /Walmart Exclusive/i.test(seriesInfoRaw) || /Walmart Exclusive/i.test(subSeriesNameRaw);
    const isKrogerExclusive = /Kroger Exclusive/i.test(seriesInfoRaw) || /Kroger Exclusive/i.test(subSeriesNameRaw);
    const isNewFor2026 = /New for 2026/i.test(seriesInfoRaw);
    ...
    if (isNewFor2026) notesParts.push('New for 2026');
    ...
    // Use Toy# for matching since it's unique per variant
    const existingVariant = await prisma.variant.findFirst({
      where: {
        modelId: model.id,
        toyNumber: toyNumber, // Match by Toy# instead of cardNumber
      },
    });
    if (existingVariant) {
    ...
    await prisma.variant.create({
      data: {
        ...
        toyNumber: toyNumber, // Store Toy# for matching
        isTreasureHunt,
        isSuperTreasureHunt,
        wheelType: undefined,
