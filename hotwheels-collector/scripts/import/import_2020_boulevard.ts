/**
 * Script to import the 2020 Hot Wheels Boulevard set into your database using Prisma.
 *
 * This script:
 *   1. Fetches the 2020 Boulevard page from Hot Wheels Fandom wiki
 *   2. Parses multiple tables (Mix 1, Mix 2, Mix 3, Mix 4, Mix 5, etc.)
 ...
 *   5. Creates database records: Year → Collection (Boulevard) → SubSeries (Mix1-5) → Model → Variant
 *
 * Boulevard-specific:
 * - No TH/STH (always false)
 * - SubSeries are Mix 1, Mix 2, Mix 3, Mix 4, Mix 5, etc.
 * - Model metadata from detail pages
 *
 * How to use:
 *   npx ts-node scripts/import/import_2020_boulevard.ts
 *
 * Notes:
 *   - The script is idempotent for Year, Collection, and SubSeries
 *   - Duplicate variant check: Variant oluşturmadan önce findFirst ile kontrol edilir (2025'teki gibi)
 *   - Model metadata is fetched from individual model pages (may take time)
 *   - Boxed Set tabloları atlanır
 */
...
const URL = 'https://hotwheels.fandom.com/wiki/2020_Hot_Wheels_Boulevard';
 ...
 * Supports: Mix 1, Mix 2, ..., Mix 5, Rerelease Mix
 */
function extractMixName($: cheerio.CheerioAPI, table: any): string {
  ...
  if (prevHeading.length > 0) {
    ...
    } else if (/rerelease\s+mix/i.test(headingText)) {
      // Match "Rerelease Mix"
      mixName = 'Rerelease Mix';
    }
  }
  ...
  if (!mixName) {
    ...
    if (mixMatch) {
      mixName = `Mix ${mixMatch[1]}`;
    } else if (/rerelease\s+mix/i.test(caption)) {
      mixName = 'Rerelease Mix';
    }
  }
  
  // Default to Mix 1 if not found
...
async function main() {
  ...
  for (let tableIdx = 0; tableIdx < tables.length; tableIdx++) {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      // Check if variant already exists (duplicate prevention - same as 2025)
