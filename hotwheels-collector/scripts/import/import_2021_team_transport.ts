/**
 * Script to import the 2021 Hot Wheels Team Transport set into your database using Prisma.
 *
 * This script:
 *   1. Fetches the 2021 Team Transport page from Hot Wheels Fandom wiki
 ...
 *
 * How to use:
 *   npx ts-node scripts/import/import_2021_team_transport.ts
 */
...
async function main() {
  ...
  for (let tableIdx = 0; tableIdx < tables.length; tableIdx++) {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      // 2021 format: Series# can be "#26" (with # prefix)
      ...
      if (firstCell.length >= 3 && firstCell.length <= 8 && /^[A-Z0-9]+$/i.test(firstCell) && !firstCell.includes(' ') &&
        ...
        if (!toyNumber || !seriesNumber) {
          ...
          if (isSupremeExclusive) {
            console.warn(`  ${subSeriesName} row ${i} cells: ${cellValues.map((v, idx) => `[${idx}]${v.substring(0, 30)}`).join(' | ')}`);
          }
          continue;
        }
    ...
    for (const [setKey, setRows] of groupedBySet.entries()) {
      ...
      // Find transport row (first row with Toy# matching the key) and car rows
      const transportRow = setRows.find(r => {
        const keyParts = setKey.split('_');
        return r.toyNumber === keyParts[0] && r.seriesNumber === keyParts[1] && 
               // Transport row should have Toy# in first cell pattern
               r.castingName && !r.castingName.includes('&');
      }) || setRows[0]; // Fallback to first row if not found
      
      const carRows = setRows.filter(r => r !== transportRow);
      ...
      // Fetch car metadata (already fetched during model creation, but get from cache)
      let metadata = modelMetadataCache.get(carRows[0].castingName);
      if (!metadata && carRows[0].castingNameLink.length > 0) {
        const modelPageHref = carRows[0].castingNameLink.attr('href');
        if (modelPageHref) {
      ...
      // Add transport information to description
      const descriptionParts: string[] = [];
      if (metadata.description) {
        descriptionParts.push(metadata.description);
      }
      ...
      // Update model description if needed
      if (modelAraba && descriptionParts.length > 0) {
      ...
      // This loop handles multiple car castings (e.g., 2 car castings = 2 variants)
      for (let i = 0; i < carRows.length; i++) {
        ...
        if (!existingCarVariant) {
          ...
          totalCreated++;
          console.log(`Created Car Variant ${i + 1}/${carRows.length}: ${carRow.castingName} for ${modelArabaName}`);
        }
      }
