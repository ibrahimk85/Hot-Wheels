/**
 * Script to import the 2023 Hot Wheels Team Transport set into your database using Prisma.
 *
 * This script:
 *   1. Fetches the 2023 Team Transport page from Hot Wheels Fandom wiki
 ...
 *
 * How to use:
 *   npx ts-node scripts/import/import_2023_team_transport.ts
 */
...
const URL = 'https://hotwheels.fandom.com/wiki/2023_Car_Culture:_Team_Transport';
...
async function main() {
  ...
  for (let tableIdx = 0; tableIdx < tables.length; tableIdx++) {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      // Debug for Supreme Exclusive / Walmart Exclusive
      ...
      // Transport row pattern: First cell is Toy# (short alphanumeric), second is Series# (digit or N/A for Supreme Exclusive/Walmart)
      // For Supreme Exclusive/Walmart, Series# can be "N/A"
      ...
      if (firstCell.length >= 3 && firstCell.length <= 8 && /^[A-Z0-9]+$/i.test(firstCell) && !firstCell.includes(' ') &&
        ...
        toyNumber = firstCell;
        // For Supreme Exclusive/Walmart with N/A, use Toy# as Series# to create unique set identifier
