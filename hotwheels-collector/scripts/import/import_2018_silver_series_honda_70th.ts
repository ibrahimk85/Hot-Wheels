/**
 * Import 2018 Hot Wheels Silver Series - Honda 70th Anniversary (Automotive category)
 * 8-col: Col # | Toy # | Casting | Color | Tampos | Wheel Type | Notes | Photo: Car | Photo: Card
 * npx ts-node scripts/import/import_2018_silver_series_honda_70th.ts
 */
...
const URL = 'https://hotwheels.fandom.com/wiki/Honda_70th_Anniversary_Series_(2018)';
...
async function main() {
  console.log(`Fetching Honda 70th Anniversary (${targetYear}) from ${URL}…`);
  const res = await fetch(URL, { headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' } });
