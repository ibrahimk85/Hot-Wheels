/**
 * Script to import the 2023 Hot Wheels Mainline set into your database using Prisma.
 *
 * This script fetches the 2023 mainline table from the Hot Wheels Fandom wiki,
 * parses each row to extract the toy number, collector number, model name,
 * sub‑series, series info (e.g. New for 2023!, Treasure Hunt, Super Treasure Hunt,
 ...
 *      setup, the 2023 mainline data will be associated with the `Year`
 *      record for 2023 and the `Collection` record named "Mainline".
 *
 *   3. Run the script with ts-node:
 *
 *         npx ts-node import_2023_mainline.ts
 *
 *      Alternatively, if you have added a script entry in your package.json
 *      (e.g. "import:2023-mainline": "ts-node import_2023_mainline.ts"), you can
 *      run it via `npm run import:2023-mainline`.
 *
 * Notes:
...
// URL of the Fandom page containing the 2023 mainline list
...
async function main() {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    const isNewFor2023 = /New for 2023/i.test(seriesInfoRaw) || /New for 2023/i.test(subSeriesNameRaw);
    ...
    if (isNewFor2023) notesParts.push('New for 2023');
