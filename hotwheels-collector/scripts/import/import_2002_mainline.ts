/**
 * Script to import the 2002 Hot Wheels Mainline set into your database using Prisma.
 *
 * Fetches ALL mainline tables from the 2002 Fandom wiki page (multi-table layout):
 * 2002 Treasure Hunts, 2002 First Editions, Segment Series (Wild Frontier, Spares 'N Strikes, Tuners,
 * Corvette, Trump Cars, Cold Blooded, Star-Spangled, Yu-Gi-Oh!, Spectraflame, Masters of the Universe,
 * Sweet Rides, Grave Rave, Red Lines, Hot Rod Magazine, Fed Fleet), 2002 Hot Wheels, Final Run.
 * Skips Walmart "Redline Exclusives" and "Goodyear Tire Exclusives" if present (same rules as 2003+).
 *
 ...
 * Final Run table is stored under SubSeries name "Final Run" (not "2002 Final Run"); Toy# overlap with open stock is handled at import + fix script.
 *
 *   npx ts-node scripts/import/import_2002_mainline.ts
 *
...
const URL = 'https://hotwheels.fandom.com/wiki/List_of_2002_Hot_Wheels';
...
async function main() {
  console.log('Fetching 2002 mainline data…');
  ...
  allTables.each((index, tableElement) => {
    ...
    // e.g. wiki cell "2002s" while h2 is "2002 Treasure Hunts"
    if (/^\d{4}s$/i.test(subSeriesName) && heading && /treasure/i.test(heading)) {
      subSeriesName = stripFandomApiArtefacts(heading);
    }
    // Final Run: wiki uses "2002 Final Run" in cells; COL# also appears in open stock — canonical SubSeries "Final Run"
    if (/\bfinal run\b/i.test(heading ?? '') || /\b2002\s+final run\b/i.test(subSeriesName)) {
  ...
  for (const tableInfo of tablesToProcess) {
    const { subSeriesName, table, headingContext } = tableInfo;
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      let cleanedSubSeriesName = stripThFromSeriesName(subSeriesNameRaw)
        ...
        .replace(/\s*New for 2002!\s*/gi, '')
        .replace(/\s*New in Mainline\s*/gi, '')
        .replace(/\s*New for 2002\s*/gi, '')
        .replace(/\s*2002\s+Final Run\s*/gi, '')
        .trim();
      ...
      const isNewFor2002 = /New for 2002/i.test(combinedText);
      ...
      if (isNewFor2002) notesParts.push('New for 2002');
