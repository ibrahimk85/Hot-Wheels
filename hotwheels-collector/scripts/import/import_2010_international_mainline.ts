/**
 * Script to import the 2010 International Hot Wheels Mainline set into your database using Prisma.
 *
 * This script fetches ALL tables from the 2010 International Hot Wheels Fandom wiki page.
 ...
 *
 *         npx ts-node import_2010_international_mainline.ts
 */
...
// URL of the Fandom page containing the 2010 International mainline list
const URL = 'https://hotwheels.fandom.com/wiki/List_of_2010_Hot_Wheels_(International)';
...
async function main() {
  console.log('Fetching 2010 International mainline data…');
  ...
  // Ensure the "Mainline (International)" SubSeries exists
  const mainlineInternationalSubSeriesName = 'Mainline (International)';
  let mainlineInternationalSubSeries = await prisma.subSeries.findFirst({
    where: {
      name: mainlineInternationalSubSeriesName,
      collectionId: collectionRecord.id,
    },
  });
  if (!mainlineInternationalSubSeries) {
    mainlineInternationalSubSeries = await prisma.subSeries.create({
      data: {
        name: mainlineInternationalSubSeriesName,
        collection: { connect: { id: collectionRecord.id } },
      },
    });
    console.log(`Created SubSeries: ${mainlineInternationalSubSeriesName}`);
  }
  ...
  for (const tableInfo of tablesToProcess) {
    ...
    // All variants will use the "Mainline (International)" SubSeries
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      // Use the "Mainline (International)" SubSeries for all variants
      const finalSubSeries = { id: mainlineInternationalSubSeries.id };

      // Determine model key
      const modelKey = collectorNumber !== null ? collectorNumberStr : `Mainline (International)-${castingName}`;
      ...
      // Check if variant already exists (by Toy# - CRITICAL for 2010)
      ...
      if (existingVariant) {
        console.log(`    Skipping duplicate variant: ${castingName} (Toy#: ${toyNumber})`);
        continue;
      }
      ...
      await prisma.variant.create({
        data: {
          ...
          year: targetYear,
          releaseName: mainlineInternationalSubSeriesName,
